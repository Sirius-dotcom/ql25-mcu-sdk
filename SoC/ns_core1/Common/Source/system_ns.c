...
__attribute__((section(".init"))) void __sync_harts(void)
{
    ...
    unsigned long hartid = hart_id.b.core_id;
    unsigned long clint_base, irgb_base, smp_base;
    unsigned long mcfg_info;

    smp_base = __SMP_BASEADDR;
    clint_base = __SYSTIMER_BASEADDR+0x1000;
...
void _premain_init(void)
{
    ...
    /* __ICACHE_PRESENT and __DCACHE_PRESENT are defined in ns.h */
...
#endif
        #if defined(MISC_HAS_USART0_HAS_CLK)
        
        usart0_clk_en(ENABLE);
        #endif

        #if defined(MISC_HAS_IOMUX_HAS_CLK)
        iomux_clk_en(ENABLE);
        #endif

        #if defined(MISC_HAS_IDU_HAS_CLK)
        idu_clk_en( ENABLE);
        #endif
    ...
    #ifdef USART_CLK_3_0_0
        usart_init_t.USART_CLK_Feq = 32000000;
    #endif
#endif
#if defined(USART_SETUP_BIT_LENGTH_8)
    #if defined(USART_SETUP_CFG_STOP_BIT_1BIT)
