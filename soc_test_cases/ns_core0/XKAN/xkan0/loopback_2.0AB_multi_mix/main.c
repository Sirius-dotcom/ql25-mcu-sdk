...
void xkan_config(void)
{
    ...
    */
    ...
    xKan_ConfigRxFifoWatermark(XKAN0, XKAN_CFG_RX_FIFO1, 0);
}
...
void XKAN0_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN0, XKAN_CFG_INT_STATUS_RXF0_WM_INT) && xKan_GetInterruptEnable(XKAN0, XKAN_CFG_INT_EN_RXF0_WM_EN))
    {
        ...
        rxfifo0_done ++;
        pRxData += BUFSIZE;
        if (rxfifo0_done == 2) xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_RXF0_WM_EN, DISABLE);
    }
...
void main(void)
{
    ...
    printf("xkan loopback_2.0AB_multi_mix test\n\r");
    /* xkan cfg */ 
    xkan_config();
