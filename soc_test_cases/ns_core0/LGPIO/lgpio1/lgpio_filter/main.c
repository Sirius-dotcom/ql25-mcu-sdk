...
{
    if(LGPIO_GetITStatus(LGPIO1,LGPIO_PinSource0,LGPIO_IT_RISE) == LGPIO_PinSource0){
        
        LGPIO_ITClear(LGPIO1,LGPIO_PinSource0,LGPIO_IT_RISE);
        state++;
    }
}
...
int main(void)
{
    ...
    iomux_config();
    LGPIO_DN_Filter_Num(LGPIO1, LGPIO_DIGITAL_NOISE_FILTER_NUM(3));
    LGPIO_Digital_Noise_Filter(LGPIO1, LGPIO_PinSource0, ENABLE);
    LGPIO_ITClear(LGPIO1,LGPIO_PinSource0,LGPIO_IT_RISE);
    LGPIO_ITConfig(LGPIO1,LGPIO_PinSource0,LGPIO_IT_RISE,ENABLE);
