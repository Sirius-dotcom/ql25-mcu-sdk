...
uint32_t QSPI1_send_array_32B[ARRAYSIZE] = {0x5aa5a5A1, 0x5aa5a5A2, 0x5aa5a5A3, 0x5aa5a5A4, 0x5aa5a5A5, 0x5aa5a5A6, 0x5aa5a5A7, 0x5aa5a5A8, 0x5aa5a5A9, 0x5aa5a5AA};
...
uint32_t QSPI1_receive_array_32B[ARRAYSIZE] = {0};
uint32_t QSPI2_receive_array_32B[ARRAYSIZE] = {0};
uint16_t QSPI1_send_array_16B[ARRAYSIZE] = {0xa5A1, 0xa5A2, 0xa5A3, 0xa5A4, 0xa5A5, 0xa5A6, 0xa5A7, 0xa5A8, 0xa5A9, 0xa5AA};
uint16_t QSPI2_send_array_16B[ARRAYSIZE] = {0xa5B1, 0xa5B2, 0xa5B3, 0xa5B4, 0xa5B5, 0xa5B6, 0xa5B7, 0xa5B8, 0xa5B9, 0xa5BA};
uint16_t QSPI1_receive_array_16B[ARRAYSIZE] = {0};
...
void main(void)
{
    ...
    if (MemoryCompare(QSPI2_receive_array, QSPI1_send_array, ARRAYSIZE) &
        MemoryCompare(QSPI1_receive_array, QSPI2_send_array, ARRAYSIZE)) {
        printf("1 SPI_master_slave_fullduplex_polling  #### PASS ####\n");
    } else {
        for (int i = 0; i < ARRAYSIZE; i++) {
            printf("data [%d]%x  %x \r\n",i,QSPI2_receive_array[i],QSPI1_receive_array[i]);
        }
        printf("1 SPI_master_slave_fullduplex_polling  #### FAIL ####\n");
    }
    for (int i = 0 ;i < ARRAYSIZE; i++) {
        QSPI1_receive_array_32B[i] = 0;
    ...
    while (send_n < ARRAYSIZE) {
        ...
        while (SET == QSPI_GetFlag(QSPI1, QSPI_STATUS_TX_FULL)) {}
        QSPI_SendData(QSPI1, QSPI1_send_array_32B[send_n++]);
        while (SET == QSPI_GetFlag(QSPI2, QSPI_STATUS_RX_EMPTY)) {}
        QSPI2_receive_array_32B[receive_n] = QSPI_ReceiveData(QSPI2);
        while (SET == QSPI_GetFlag(QSPI1, QSPI_STATUS_RX_EMPTY)) {}
        QSPI1_receive_array_32B[receive_n++] = QSPI_ReceiveData(QSPI1);
        QSPI_SSMSwitch(QSPI2,DISABLE);
    }
    if (MemoryCompare32B(QSPI2_receive_array_32B, QSPI1_send_array_32B, ARRAYSIZE) &
        MemoryCompare32B(QSPI1_receive_array_32B, QSPI2_send_array_32B, ARRAYSIZE)) {
        printf("2 SPI_master_slave_fullduplex_polling  #### PASS ####\n");
    } else {
        printf("2 SPI_master_slave_fullduplex_polling  #### FAIL ####\n");
    }
    for(int i = 0; i < ARRAYSIZE; i++) {
        QSPI1_send_array_16B[i] = rand() % 65535;
        QSPI2_send_array_16B[i] = rand() % 65535;
    }
    for (int i = 0; i < ARRAYSIZE; i++) {
        QSPI1_receive_array_16B[i] = 0;
    ...
    while (send_n < ARRAYSIZE) {
        ...
        while (SET == QSPI_GetFlag(QSPI1, QSPI_STATUS_TX_FULL)) {}
        QSPI_SendData(QSPI1, QSPI1_send_array_16B[send_n++]);
        while (SET == QSPI_GetFlag(QSPI2, QSPI_STATUS_RX_EMPTY)) {}
        QSPI2_receive_array_16B[receive_n] = QSPI_ReceiveData(QSPI2);
        while (SET == QSPI_GetFlag(QSPI1, QSPI_STATUS_RX_EMPTY)) {}
        QSPI1_receive_array_16B[receive_n++] = QSPI_ReceiveData(QSPI1);
        QSPI_SSMSwitch(QSPI2,DISABLE);
    }
    if (MemoryCompare16B(QSPI2_receive_array_16B, QSPI1_send_array_16B, ARRAYSIZE) &
        MemoryCompare16B(QSPI1_receive_array_16B, QSPI2_send_array_16B, ARRAYSIZE)) {
