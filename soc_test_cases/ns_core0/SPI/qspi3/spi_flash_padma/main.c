...
{
    Spi_NorFlash_Write_Enable(QSPI3);
    Spi_NorFlash_CS(QSPI3, CS_ON);
    SPI_WriteByte(QSPI3, cmd);
    SPI_WriteByte(QSPI3, addr >> 16);
    SPI_WriteByte(QSPI3, addr >> 8);
    SPI_WriteByte(QSPI3, addr);

    UDMA_PAM2MTypeDef UDMA_PAM2MStruct = {0};
    UDMA_PAM2MStruct.UDMA_TransEn = PA2M_TRANS_ENABLE;
    UDMA_PAM2MStruct.UDMA_DstBaseAddr = CAL_ADDR_LO(&(QSPI3->TXDATA));
    UDMA_PAM2MStruct.UDMA_SrcBaseAddr = CAL_ADDR_LO(src_base_addr);
#if defined(SOC_ADDRESS_HIGH_VALID) && SOC_ADDRESS_HIGH_VALID == 1
    UDMA_PAM2MStruct.UDMA_DstBaseAddr_H = CAL_ADDR_HI(&(QSPI3->TXDATA));
    ...
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_QSPI3_TX_DMA;  
    UDMA_PAM2M_Init(QSPI3_TX_DMA_DMA_CH, &UDMA_PAM2MStruct);

    QSPI_TxDmaStart(QSPI3, ENABLE);

    while (!UDMA_PA2M_GetITStatus(QSPI3_TX_DMA_DMA_IRQ, PA2M_FTRANS_IRQ_STAT)) {}
    while (QSPI_GetFlag(QSPI3,QSPI_STATUS_BUSY) == SET) {}
    Spi_NorFlash_CS(QSPI3, CS_OFF);
    QSPI_TxDmaStart(QSPI3, DISABLE);
    Spi_NorFlash_Wait_Busy(QSPI3);
}
...
void SPI_PageRead_Dma(uint8_t cmd, uint32_t addr)
{
    ...
    SPI_WriteByte(QSPI3, addr >> 8);
    SPI_WriteByte(QSPI3, addr);
    SPI_WriteByte(QSPI3, DUMMY_BYTE);
    ...
    UDMA_PAM2MStruct.UDMA_Width = PA2M_MDWIDTH_8BIT;
    UDMA_PAM2MStruct.UDMA_Mode = PA2M_MODE_NORMAL;
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_QSPI3_RX_DMA;  
    UDMA_PAM2M_Init(QSPI3_RX_DMA_DMA_CH, &UDMA_PAM2MStruct);
    ...
    QSPI_RxDmaStart(QSPI3, ENABLE);
    QSPI_TxDmaStart(QSPI3, ENABLE);

    while (!UDMA_PA2M_GetITStatus(QSPI3_RX_DMA_DMA_IRQ, PA2M_FTRANS_IRQ_STAT)) {}
    Spi_NorFlash_CS(QSPI3, CS_OFF);
    QSPI_TxDmaStart(QSPI3, DISABLE);
    QSPI_RxDmaStart(QSPI3, DISABLE);
}
...
void main(void)
{
    ...
    UDMA_PA2M_ITConfig(QSPI3_RX_DMA_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
    UDMA_PA2M_ITConfig(QSPI3_TX_DMA_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
