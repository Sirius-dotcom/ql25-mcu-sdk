...
void SPI_PageWrite(uint8_t cmd, uint8_t* buf, uint32_t addr)
{
    ...
    QSPI_TxsizeConfig(QSPI3, FLASH_PAGESIZE);
    
    for(int i = 0; i < FIFO_DP; i++) {
        QSPI3->TXDATA = write_buf[i];
    }

    for(int j = FIFO_DP; j < FLASH_PAGESIZE ; j++) {
        while (QSPI_GetFlag(QSPI3, QSPI_STATUS_TX_FULL)) {}
        QSPI3->TXDATA = write_buf[j];
    }
    while(!QSPI_GetFlag(QSPI3, QSPI_STATUS_TX_DONE)){}
...
void SPI_PageRead(uint8_t cmd, uint8_t* buf, uint32_t addr)
{
    ...
    QSPI_BusWidthSwitch(QSPI3, QSPI_FMT_PROTO_QUAD);
    QSPI_DirectionConfig(QSPI3, QSPI_FMT_DIR_RX);

    QSPI_RxsizeConfig(QSPI3, FLASH_PAGESIZE);
    
    for(int j = 0; j < FIFO_DP; j++) {
        QSPI3->TXDATA = Dummy_Byte; 
        
    }

    for (int i = 0; i < FLASH_PAGESIZE; i++) {
        while (QSPI3->STATUS & QSPI_STATUS_RX_EMPTY);
        buf[i] = (QSPI3->RXDATA & 0xFF);
        if(i < FLASH_PAGESIZE - FIFO_DP) {
            QSPI3->TXDATA = Dummy_Byte; 
        }
    }

    while(!QSPI_GetFlag(QSPI3, QSPI_STATUS_RX_DONE)) {}
...
{
    uint32_t ip_st = QSPI3->IP;
    uint32_t ie_st = QSPI3->IE;
    switch (ie_st)
    {
        case QSPI_IE_RX:
            if (ip_st & QSPI_IP_RX) {
                read_buf[i_rx] = (QSPI3->RXDATA & 0xFF);
                i_rx ++;
                read_buf[i_rx] = (QSPI3->RXDATA & 0xFF);
                i_rx ++;
                read_buf[i_rx] = (QSPI3->RXDATA & 0xFF);
                ...
                if(i_rx == FLASH_PAGESIZE) {
                    QSPI_ITConfig(QSPI3, QSPI_IE_RX, DISABLE);
                }
            }
            break;
        case QSPI_IE_TX:
            if (ip_st & QSPI_IP_TX) {
                for(int i =0;i<TX_WATERMARK;i++) {
                    QSPI3->TXDATA = write_buf[i_tx];
                    i_tx ++;
                }
                if(i_tx == FLASH_PAGESIZE) {
                    QSPI_ITConfig(QSPI3, QSPI_IE_TX, DISABLE);
                }
            }
