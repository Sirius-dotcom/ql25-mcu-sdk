...
{
    if (RESET != QSPI_GetIPStatus(QSPI3, QSPI_IP_TX)) {
        /* send data */
    ...
    } else if (SET == QSPI_GetFlag(QSPI3, QSPI_STATUS_RX_OVR)) {
        QSPI_ClearFlag(QSPI3,QSPI_STATUS_RX_OVR);
        printf("this is QSPI_STATUS_RX_OVR\r\n");
    } else if (SET == QSPI_GetFlag(QSPI3, QSPI_STATUS_TX_UDR)) {
...
void main(void)
{
    ...
    QSPI_RxmarkConfig(QSPI3, 0);
    /* QSPI interrupt enable */
    QSPI_ITConfig(QSPI2, QSPI_IE_RX,ENABLE);
    QSPI_ITConfig(QSPI3, QSPI_IE_TX,ENABLE);
    /* wait transmit complete */
    while (receive_n < ARRAYSIZE) {}
    /* compare receive data with send data */
    if (MemoryCompare(QSPI2_send_array, QSPI3_send_array, ARRAYSIZE)) {
        simulation_pass();
    } else {
        simulation_fail();
    }
