...
/**
 * \brief this function handles QSPI3 Handler exception
 */
void QSPI3_IRQHandler(void)
{
    if ((SET == QSPI_GetFlag(QSPI3, QSPI_STATUS_RX_UDR)) & 
        (SET == QSPI_GetITConfig(QSPI3, QSPI_IE_RX_UDR))) {
        QSPI_ClearFlag(QSPI3,QSPI_STATUS_RX_UDR);
        test_cs = 1;
        
        QSPI_ITConfig(QSPI3, QSPI_IE_RX_UDR,DISABLE);
    }
    if ((SET == QSPI_GetFlag(QSPI3, QSPI_STATUS_TX_UDR)) & 
        (SET == QSPI_GetITConfig(QSPI3, QSPI_IE_TX_UDR))) {
        QSPI_ClearFlag(QSPI3,QSPI_STATUS_TX_UDR);
        test_cs = 3;
        
        QSPI_ITConfig(QSPI3, QSPI_IE_TX_UDR,DISABLE);
    }
    if ((SET == QSPI_GetFlag(QSPI3, QSPI_STATUS_TX_OVR)) & 
        (SET == QSPI_GetITConfig(QSPI3, QSPI_IE_TX_OVR))) {
        QSPI_ClearFlag(QSPI3,QSPI_STATUS_TX_OVR);
        test_cs = 5;
        
        QSPI_ITConfig(QSPI3, QSPI_IE_TX_OVR,DISABLE);
    }
    if ((SET == QSPI_GetFlag(QSPI3, QSPI_STATUS_RX_OVR)) & 
        (SET == QSPI_GetITConfig(QSPI3, QSPI_IE_RX_OVR))) {
        QSPI_ClearFlag(QSPI3,QSPI_STATUS_RX_OVR);
        test_cs = 7;
        
        QSPI_ITConfig(QSPI3, QSPI_IE_RX_OVR,DISABLE);
    }
}
...
void main(void)
{
    ...
    /* SPI configure */
    SPI_Config();
    QSPI_TxmarkConfig(QSPI3, 1);
    QSPI_RxmarkConfig(QSPI2, ARRAYSIZE);
    QSPI_TxmarkConfig(QSPI2, 1);
    QSPI_RxmarkConfig(QSPI3, ARRAYSIZE);

    while(1) {
        switch (test_cs) {
            case 0:
                test_cs = 0xFF;
                QSPI_ITConfig(QSPI3, QSPI_IE_RX_UDR,ENABLE);
                QSPI_ReceiveData(QSPI3);
                break;
            case 1:
                test_cs = 0xFF;
            ...
            case 2:
                #if 0
                test_cs = 0xFF;
                QSPI_ITConfig(QSPI3, QSPI_IE_TX_UDR,ENABLE);
            ...
            case 3:
                ...
                QSPI_SendData(QSPI3, 0xAA);
                while (QSPI3->STATUS & QSPI_STATUS_BUSY) {}
                break;
            case 4:
                test_cs = 0xFF;
                QSPI_ITConfig(QSPI3, QSPI_IE_TX_OVR,ENABLE);
                QSPI_DirectionConfig(QSPI3, QSPI_FMT_DIR_TX);
            ...
            case 6:
                test_cs = 0xFF;
                QSPI_ITConfig(QSPI3, QSPI_IE_RX_OVR,ENABLE);
