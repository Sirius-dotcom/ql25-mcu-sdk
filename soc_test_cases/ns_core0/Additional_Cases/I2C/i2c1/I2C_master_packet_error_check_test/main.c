...
void i2c_pec_transceive_test_autoend(void)
{
    ...
    for (int i = 0; i < BUFSIZE; i++) {
        ...
        I2C1->TXDATA = i2c_mst_send1[i];
        while (I2C_Get_Status(I2C1, I2C_STATUS_TXFIFO_FULL)) {}
    ...
    pec_byte_send[0] = I2C_Get_PECode(I2C1);
...
void I2C1_IRQHandler(void)
{
    ...
    {
        pec_byte_send[1] = I2C_Get_PECode(I2C1);
...
void i2c_pec_transceive_test_unautoend(void)
{
    ...
    for (int i = 0; i < BUFSIZE; i++) {
        /* send a data byte to i2c bus */
        I2C1->TXDATA = (end_flag ? i2c_mst_send3[i] : i2c_mst_send2[i]);
        while (I2C_Get_Status(I2C1, I2C_STATUS_TXFIFO_FULL)) {}
        while (I2C_Get_Status(I3, I2C_STATUS_RXFIFO_EMPTY)) {}
        if (end_flag) {
    ...
    if(end_flag == 1) {
        while (!I2C_Get_Status(I2C1, I2C_STATUS_EOT)) {}
        I2C_Clear_Status(I2C1, I2C_STATUS_EOT);
        pec_byte_send[2] = I2C_Get_PECode(I2C1);
