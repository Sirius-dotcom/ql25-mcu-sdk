...
void i2c_pec_transceive_test_autoend(void)
{
    /* pec request */
    I2C_PEC_Request(I3);
    I2C_PEC_Request(I2C1);

    /* send slave address to I2C bus */
    I2C_Send_SlaveAddr(I2C1, I3_SLAVE_ADDRESS1, I2C_DIR_READ, 0xFFFFFFFF);
    /* wait for addr match */
    ...
    for (int i = 0; i < BUFSIZE; i++) {
        ...
        i2c_mst_recv1[i] = I2C1->RXDATA;
    }

    /* wait for controller exit from ack stage*/
    while(I2C_CHECK_MST_STATE(I2C1, I2C_STATUS_MASTER_STATE_ACK));
    ...
    pec_byte_recv[0] = I2C_Get_PECode(I2C1);
    while(!I2C_Get_Status(I2C1, I2C_STATUS_EOT));
    I2C_Clear_Status(I2C1, I2C_STATUS_EOT );

    /* wait for controller exit from stop stage*/
    while(I2C_CHECK_MST_STATE(I2C1, I2C_STATUS_MASTER_STATE_STOP));
}
...
void I3_IRQHandler(void)
{
    if(I2C_Get_IntStatus(I3, I2C_INT_IE_EOT_ENABLE) && I2C_Get_Status(I3, I2C_STATUS_EOT))
    {
        ...
        pec_byte_recv[1] = I2C_Get_PECode(I2C1);
...
void i2c_pec_transceive_test_unautoend(void)
{
    ...
    for (int i = 0; i < BUFSIZE; i++) {
        ...
        while (I2C_Get_Status(I2C1, I2C_STATUS_RXFIFO_EMPTY)) {}
        if (end_flag) {
            i2c_mst_recv3[i] = I2C1->RXDATA;
        } else {
            i2c_mst_recv2[i] = I2C1->RXDATA;
        }
    }
    if(end_flag == 1) {
        while (!I2C_Get_Status(I2C1, I2C_STATUS_EOT)) {}
        pec_byte_send[2] = I2C_Get_PECode(I3);
        pec_byte_recv[2] = I2C_Get_PECode(I2C1);
        I2C_Clear_Status(I2C1, I2C_STATUS_EOT);
    }
    do {
