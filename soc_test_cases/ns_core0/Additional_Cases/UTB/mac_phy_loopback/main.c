...
static int udma_m2m_transfer(const void *src, void *dst, uint32_t len, uint32_t src_inc, uint32_t dst_inc)
{
    ...
    udma_init.UDMA_SrcWidth = UDMA_MSWIDTH_8BIT;
    udma_init.UDMA_DstWidth = UDMA_MDWIDTH_8BIT;
...
{
    volatile uint8_t *const mac_tx = (volatile uint8_t *)UTB_MAC_TX_SYMB;
    volatile uint8_t *const mac_rx = (volatile uint8_t *)UTB_MAC_RX_SYMB;
    unsigned long timeout = UTB_LOOPBACK_WAIT;

    /* Assumption: mac_tx_symb/mac_rx_symb are 8-bit streaming ports. */
    if (!udma_m2m_transfer(src_buf, (void *)mac_tx, PKT_LEN, UDMA_MSNA_ENABLE, UDMA_MDNA_DISABLE)) {
        return 0;
    }
    ...
    if (!udma_m2m_transfer((const void *)mac_rx, dst_buf, PKT_LEN, UDMA_MSNA_DISABLE, UDMA_MDNA_ENABLE)) {
        return 0;
    }
