...
void XKAN1_IRQHandler(void)
{
    ...
    if(xKan_GetInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_RXF0_WM_INT) && xKan_GetInterruptEnable(XKAN1, XKAN_CFG_INT_EN_RXF0_WM_EN))
    {
        ...
        rx_done ++;
        pRxData += BUFSIZE;
        if (rx_done == 1) xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_RXF0_WM_EN, DISABLE);
    }
...
void main(void)
{
    ...
    sleep_status = xKan_Sleep(XKAN1, ENABLE);
    ...
    do {
        wakeup_status = xKan_GetMsrStatus(XKAN1, XKAN_CFG_MSR_WAKE_UP);
    } while (!wakeup_status);
    XKAN1->CFG_MSR |= XKAN_CFG_MSR_WAKE_UP;
