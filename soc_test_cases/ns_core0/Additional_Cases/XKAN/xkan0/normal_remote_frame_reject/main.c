...
void xkan_config(void)
{
    ...
    */
    ...
    xKan_ConfigGlobalFilter(XKAN1, XKAN_REJECT, XKAN_REJECT, XKAN_REJECT_REMOTE, XKAN_REJECT_REMOTE);
...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    xKan_EnableTxBufferRequest(XKAN0, XKAN_TX_BUFFER0|XKAN_TX_BUFFER1|XKAN_TX_BUFFER2);
}
...
void XKAN1_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_RXF0_WM_INT) && xKan_GetInterruptEnable(XKAN1, XKAN_CFG_INT_EN_RXF0_WM_EN))
    {
        ...
        rx2_done ++;
        if (rx2_done == 1) xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_RXF0_WM_EN, DISABLE);
    }
}
