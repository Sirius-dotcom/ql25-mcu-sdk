...
void XKAN0_IRQHandler(void)
{
    ...
    {
        xKan_RxBufferNewMessageCallback(XKAN0, &xkan_parameter_init, &xKan_RxHeader, pRxData2, XKAN0_RAM_BASE);
        rx1_done ++;
        pRxData2 += BUFSIZE;
        if (rx1_done == 4 ) xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void XKAN1_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_RXB_UPD_INT) && xKan_GetInterruptEnable(XKAN1, XKAN_CFG_INT_EN_RXB_UPD_EN))
    {
        ...
        rx2_done ++;
        pRxData += BUFSIZE;
        if (rx2_done == 4 ) xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
