...
volatile ErrStatus tdc_err_flag = ERROR;

void xkan_config(ControlStatus tdc_ena)
{
    /* step0: clk config */
    ...
    xkan_parameter_init.tx_delay_compensation = tdc_ena;
    ...
    xkan_parameter_init.data_time.DataTimeSeg1 = 0x2;
...
void main(void)
{
    ...
    printf("delay:%d\n",(XKAN0->CFG_PSR & XKAN_CFG_PSR_TRANS_DELAY_COMP)>>16);
    ...
    while (tdc_err_flag == ERROR) {}
    
    /* compare the read buffer and write buffer */
    if(SUCCESS != MemoryCompare(tx_msg, g_KanRxData, BUFSIZE) || (SUCCESS != tdc_err_flag)) {
        simulation_fail();
    } else {
        simulation_pass();
    }
