...
void xkan2_SendClassicPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    if(_Len == XKAN_DLC_BYTES_8) {
        XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN2, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER17, XKAN2_RAM_BASE);
        xKan_EnableTxBufferRequest(XKAN2, XKAN_TX_BUFFER17);
        while ((XKAN2->CFG_ESR & XKAN_CFG_ESR_TEC) < 128);
...
void xkan2_SendCanFdPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    /* step1: cfg send frame */
    ...
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN2, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER16, XKAN2_RAM_BASE);

    xKan_EnableTxBufferRequest(XKAN2, XKAN_TX_BUFFER16);
}
...
void XKAN2_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_EN_BUS_OFF_INT_EN) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_STATUS_BUS_OFF_INT))
    {
        ...
        rec_cnt[1] = (XKAN2->CFG_ESR & XKAN_CFG_ESR_REC >> 8);
    }
}
...
void main(void)
{
    ...
    xkan2_SendClassicPacket(tx_msg, XKAN_DLC_BYTES_1);
    xkan2_SendCanFdPacket(tx_msg, XKAN_DLC_BYTES_1);
    while ((XKAN2->CFG_ESR & XKAN_CFG_ESR_TEC) != 127);

    /* xkan2 rec up to 128 */
    xkan1_SendClassicPacket(tx_msg, XKAN_DLC_BYTES_1);

    /* xkan2 seng a frame with passive err */
    xkan2_SendClassicPacket(tx_msg, XKAN_DLC_BYTES_8);
