...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    if(_Len == XKAN_DLC_BYTES_0){
        xKan_TxHeader.TxFrameType = XKAN_REMOTE_FRAME;
    } else if(_Len == XKAN_DLC_BYTES_8) {
...
void XKAN2_IRQHandler(void)
{
    ...
    {
        xKan_RxBufferNewMessageCallback(XKAN2, &xkan_parameter_init, &xKan_RxHeader, tmpData, XKAN2_RAM_BASE);
        if(xKan_RxHeader.RxFrameType == 0x40000000) xKan_SendPacket(tx_msg, XKAN_DLC_BYTES_8);;
        rx2_done ++;
...
void main(void)
{
    ...
    printf("xkan normal_remote_frame_filter\n\r");
    /* xkan cfg */ 
    iomux_config();
    xkan_config();
    ...
    xKan_SendPacket(tx_msg, XKAN_DLC_BYTES_0);
