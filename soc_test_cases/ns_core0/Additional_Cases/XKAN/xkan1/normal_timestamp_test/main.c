...
uint16_t timestamp_t[FRAMENUM] = {0};
uint16_t timestamp_r[FRAMENUM] = {0};
uint16_t timestamp_golden[FRAMENUM] = {0x6e, 0x70, 0x82, 0x80};
...
void XKAN2_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXB_UPD_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXB_UPD_EN))
    {
        ...
        timestamp_r[rx_done] = xKan_GetTimeStamp(XKAN2);
        rx_done ++;
        pRxData += BUFSIZE;
...
void XKAN1_IRQHandler(void)
{
    ...
    {
        xKan_ClearTxComplete(XKAN1);
        timestamp_t[tx_done] = xKan_GetTimeStamp(XKAN1);
        tx_done ++;
        if (tx_done == 4 ) xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_TX_TRANS_COMPLETE_EN, DISABLE);
    }
}
...
ErrStatus timestamp_check(uint16_t *timestamp_t, uint16_t *timestamp_r)
{
    uint16_t timestamp_dt[FRAMENUM];
    uint16_t timestamp_dr[FRAMENUM];
    timestamp_dt[0] = timestamp_t[0];
    timestamp_dr[0] = timestamp_r[0];
    for(int i = 1; i < FRAMENUM; i++) {

        timestamp_dt[i] = timestamp_t[i] - timestamp_t[i-1];
        timestamp_dr[i] = timestamp_r[i] - timestamp_t[i-1];
    }

    for(int k = 0; k < FRAMENUM; k++) {
        if(timestamp_dt[k] != timestamp_golden[k] || timestamp_dr[k] != timestamp_golden[k]) {
            return ERROR;
        }
    }
    return SUCCESS;
...
void main(void)
{
    ...
    while(rx_done != FRAMENUM || tx_done != FRAMENUM) {}

    /* compare the read buffer and write buffer */
    if(SUCCESS != MemoryCompare(g_KanTxData, g_KanRxData, BUFSIZE * FRAMENUM) || SUCCESS != timestamp_check(timestamp_t, timestamp_r)) {
        simulation_fail();
    } else {
        simulation_pass();
    }
