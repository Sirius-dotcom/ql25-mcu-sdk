...
{
    ErrStatus state;
    ErrStatus clk_stop_status1 = ERROR;
    ErrStatus clk_stop_status2 = ERROR;
    ...
    printf("xkan normal_stop_send_test \n\r");
    /* xkan cfg */ 
    xkan_config();
    ...
    * 1.clk stop req 
    * 2.wait clk stop ack 
    * 3.disable xkan clock enter into low-power mode
    * 4.enable xkan clock after a mount
    * 5.clear status
    * */
    clk_stop_status1= xKan_ClkStop(XKAN1, ENABLE);
    clk_stop_status2 = xKan_ClkStop(XKAN2, ENABLE);

    clk_stop_status1 &= xKan_ClkStop(XKAN1, DISABLE);
    clk_stop_status2 &= xKan_ClkStop(XKAN2, DISABLE);

    /* wait for read over for rx_buffer */
    while(rx_done == 0){}

    /* compare the read buffer and write buffer */
    if(SUCCESS != MemoryCompare(g_KanTxData, g_KanRxData, BUFSIZE) || SUCCESS != clk_stop_status1 || SUCCESS != clk_stop_status2 ) {
        simulation_fail();
    } else {
        simulation_pass();
    }
