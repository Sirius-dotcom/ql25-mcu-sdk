...
void xkan_config(void)
{
    ...
    xkan_parameter_init.ram_info.ExtFiltersNbr = 64;
    ...
    for(int j = 0; j < 64; j++){
        ...
        xKan_RXFilter.FilterType = XKAN_FILTER_MASK;
        xKan_RXFilter.FilterConfig = XKAN_FILTER_TO_RXBUFFER;
        xKan_RXFilter.FilterID1 = 0x10000000 + j;
...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    xKan_TxHeader.Identifier = 0x07F;
    ...
    xKan_TxHeader.Identifier = 0x1000003F;
...
void XKAN2_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXB_UPD_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXB_UPD_EN))
    {
        ...
        rx_done ++;
        pRxData += BUFSIZE;
        if (rx_done == 2 ) xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void main(void)
{
    ...
    printf("xkan normal_multi_filter test\n\r");
    /* xkan cfg */ 
    xkan_config();
    ...
    if(SUCCESS != MemoryCompare(g_KanTxData, g_KanRxData, BUFSIZE * 2)) {
        simulation_fail();
    } else {
        simulation_pass();
    }
