...
void xkan_config(void)
{
    ...
    */
    ...
    xKan_ConfigRxFifoWatermark(XKAN2, XKAN_CFG_RX_FIFO0, 1);
    xKan_ConfigRxFifoWatermark(XKAN2, XKAN_CFG_RX_FIFO1, 1);
}
...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER4, XKAN1_RAM_BASE);

    xKan_TxHeader.Identifier = 0x15;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER5, XKAN1_RAM_BASE);
    xKan_TxHeader.Identifier = 0x16;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER6, XKAN1_RAM_BASE);
    xKan_TxHeader.Identifier = 0x17;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER7, XKAN1_RAM_BASE);
    xKan_TxHeader.Identifier = 0x18;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER8, XKAN1_RAM_BASE);

    while (1)
    {
        xKan_EnableTxBufferRequest(XKAN1, 0x1FF);
        do {
            tmpreg = XKAN1->CFG_TXB_TRANS;
        } while (tmpreg != 0x1FF);
        XKAN1->CFG_TXB_TRANS = tmpreg;

        if(xKan_GetInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_TIME_STAMP_WRAP_INT)) {
            xKan_ClearInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_TIME_STAMP_WRAP_INT);
            break;
        }
    }
}
...
void XKAN2_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF0_WM_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF0_WM_EN))
    {
        ...
        xKan_ConfigRxFifoWatermark(XKAN2, XKAN_CFG_RX_FIFO0, 2);
    }

    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF0_FILL_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF0_FILL_EN))
    {
        interrupt_staus |= XKAN_CFG_INT_STATUS_RXF0_FILL_INT;
        xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF0_FILL_EN, DISABLE);
        xKan_ClearInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF0_FILL_INT);
    }

    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF0_FULL_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF0_FULL_EN))
    {
        interrupt_staus |= XKAN_CFG_INT_STATUS_RXF0_FULL_INT;
        xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF0_FULL_EN, DISABLE);
    }

    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF0_LOST_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF0_LOST_EN))
    {
        interrupt_staus |= XKAN_CFG_INT_STATUS_RXF0_LOST_INT;
        xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF0_LOST_EN, DISABLE);
    }
    ...
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF1_WM_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF1_WM_EN))
    {
        ...
        xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF1_WM_EN, DISABLE);
        xKan_ConfigRxFifoWatermark(XKAN2, XKAN_CFG_RX_FIFO1, 2);
    }
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF1_FILL_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF1_FILL_EN))
    {
        interrupt_staus |= XKAN_CFG_INT_STATUS_RXF1_FILL_INT;
        xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF1_FILL_EN, DISABLE);
        xKan_ClearInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF1_FILL_INT);
    }

    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF1_FULL_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF1_FULL_EN))
    {
        interrupt_staus |= XKAN_CFG_INT_STATUS_RXF1_FULL_INT;
        xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF1_FULL_EN, DISABLE);
    }

    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF1_LOST_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF1_LOST_EN))
    {
        interrupt_staus |= XKAN_CFG_INT_STATUS_RXF1_LOST_INT;
        xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF1_LOST_EN, DISABLE);
    }
    ...
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXB_UPD_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXB_UPD_EN))
    {
        ...
        xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void XKAN1_IRQHandler(void)
{
    ...
    if(xKan_GetInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_TX_TRANS_COMPLETE_INT) && xKan_GetInterruptEnable(XKAN1, XKAN_CFG_INT_EN_TX_TRANS_COMPLETE_EN))
    {
        interrupt_staus |= XKAN_CFG_INT_STATUS_TX_TRANS_COMPLETE_INT;
        xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_TX_TRANS_COMPLETE_EN, DISABLE);

    }

    if(xKan_GetInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_TIME_STAMP_WRAP_INT) && xKan_GetInterruptEnable(XKAN1, XKAN_CFG_INT_EN_TIME_STAMP_WRAP_EN))
    {
        interrupt_staus |= XKAN_CFG_INT_STATUS_TIME_STAMP_WRAP_INT;
        xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_TIME_STAMP_WRAP_EN, DISABLE);
    }
}
...
void main(void)
{
    ...
    /* enable XKAN2 receive corresponding interrupt */
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF0_FILL_EN, ENABLE);
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF0_WM_EN, ENABLE);
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF0_FULL_EN, ENABLE);
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF0_LOST_EN, ENABLE);
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF1_FILL_EN, ENABLE);
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF1_WM_EN, ENABLE);
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF1_FULL_EN, ENABLE);
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF1_LOST_EN, ENABLE);
    xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXB_UPD_EN, ENABLE);
    /* enable XKAN1 interrupt */
    
    xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_TX_TRANS_COMPLETE_EN, ENABLE);
    xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_TIME_STAMP_WRAP_EN, ENABLE);
