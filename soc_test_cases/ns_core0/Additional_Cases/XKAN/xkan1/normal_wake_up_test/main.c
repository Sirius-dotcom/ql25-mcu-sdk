...
void xkan_config(void)
{
    ...
    xkan_parameter_init.ram_info.RxBufferSize = XKAN_DATA_BYTES_24;

    xkan_parameter_init.ram_info.TxBuffersNbr = 4;
    xkan_parameter_init.ram_info.TxElmtSize = XKAN_DATA_BYTES_24;
...
void XKAN2_IRQHandler(void)
{
    ...
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXF0_WM_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXF0_WM_EN))
    {
        ...
        rx_done ++;
        pRxData += BUFSIZE;
        if (rx_done == 1) xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXF0_WM_EN, DISABLE);
    }
...
{
    ErrStatus state;
    ErrStatus sleep_status = ERROR;
    ErrStatus wakeup_status = ERROR;
    ...
    sleep_status = xKan_Sleep(XKAN2, ENABLE);
    ...
    /*send tx_msg */
    xKan_SendPacket(tx_msg, XKAN_DLC_BYTES_24);

    do {
        wakeup_status = xKan_GetMsrStatus(XKAN2, XKAN_CFG_MSR_WAKE_UP);
    } while (!wakeup_status);
    XKAN2->CFG_MSR |= XKAN_CFG_MSR_WAKE_UP;

    /* bus-sync waked up controller, sleep-req cleared by hardware */
    ...
    if(SUCCESS != MemoryCompare(g_KanTxData, g_KanRxData, BUFSIZE) || SUCCESS != sleep_status || SET != wakeup_status ) {
        simulation_fail();
    } else {
        simulation_pass();
    }
