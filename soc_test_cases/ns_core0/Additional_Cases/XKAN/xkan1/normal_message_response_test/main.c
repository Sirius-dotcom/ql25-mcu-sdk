...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    _DataBuf = tmp_msg2;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER30, XKAN1_RAM_BASE);

    xKan_TxHeader.Identifier = 0x12345679;
    xKan_TxHeader.IdType = XKAN_EXTENDED_ID;
    _DataBuf = tmp_msg3;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER31, XKAN1_RAM_BASE);

    xKan_EnableTxBufferRequest(XKAN1, XKAN_TX_BUFFER0|XKAN_TX_BUFFER1|XKAN_TX_BUFFER30|XKAN_TX_BUFFER31);
}
