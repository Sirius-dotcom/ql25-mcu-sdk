...
void ADVANCED_TIMER2_IRQHandler(void)
{
    ...
    if(Timer_FlagGet(ADVANCED_TIMER2, ADV_TIMER_TIM_SR_TIF) == SET)
    {
        state0 = 0;
        Timer_InterruptFlagClear(ADVANCED_TIMER2, ADV_TIMER_TIM_SR_TIF);
    }
...
{
    if (UDMA_PA2M_GetITStatus(ADVANCED_TIMER2_TRG_DMA_IRQ, PA2M_FTRANS_IRQ_STAT) == SET) {
        Timer_DmaEnable(ADVANCED_TIMER2, ADV_TIMER_TIM_DIER_TDE_DISABLE);
        state1 = 0;
        UDMA_PA2M_ClearITStatus(ADVANCED_TIMER2_TRG_DMA_IRQ, PA2M_FTRANS_IRQ_CLEAR_STAT);
    }
}
...
void TIMER_Config(void)
{
    ...
    Timer_ChannelOutputPulseValueConfig(ADVANCED_TIMER2, TIMER_CH_3, 500);
    ...
    Timer_InterruptEnable(ADVANCED_TIMER2, ADV_TIMER_TIM_DIER_TIE | ADV_TIMER_TIM_DIER_TDE);  
    ECLIC_Register_IRQ(ADVANCED_TIMER2_IRQn, ECLIC_NON_VECTOR_INTERRUPT, ECLIC_LEVEL_TRIGGER, 1, 0, ADVANCED_TIMER2_IRQHandler);

    Timer_DmaEnable(ADVANCED_TIMER2, ADV_TIMER_TIM_DIER_TDE);
...
void main(void)
{
    ...
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_ADVANCED_TIMER2_TRG;
    UDMA_PAM2M_Init(ADVANCED_TIMER2_TRG_DMA_CH, &UDMA_PAM2MStruct);

    ECLIC_Register_IRQ(UDMA0_IRQn, ECLIC_NON_VECTOR_INTERRUPT,
                    ECLIC_LEVEL_TRIGGER, 1, 0,
                    UDMA0_IRQHandler);

    UDMA_PA2M_ITConfig(ADVANCED_TIMER2_TRG_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
