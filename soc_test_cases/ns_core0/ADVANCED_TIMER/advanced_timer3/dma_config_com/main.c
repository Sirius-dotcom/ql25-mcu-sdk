...
#define ADVANCED_TIMER3_CH0CV_ADDR       (ADVANCED_TIMER3_BASE + 0x4c)
#define ARR_SIZE            (3)
...
{
     if (UDMA_PA2M_GetITStatus(ADVANCED_TIMER3_COM_DMA_IRQ, PA2M_FTRANS_IRQ_STAT) == SET) {
        Timer_DmaEnable(ADVANCED_TIMER3, ADV_TIMER_TIM_DIER_COMDE_DISABLE);
        state1=0;
        UDMA_PA2M_ClearITStatus(ADVANCED_TIMER3_COM_DMA_IRQ, PA2M_FTRANS_IRQ_CLEAR_STAT);
     }
}
void ADVANCED_TIMER3_IRQHandler(void)
{

    if(Timer_FlagGet(ADVANCED_TIMER3, ADV_TIMER_TIM_SR_COMIF) == SET)
    {
        state0 = 0;
        Timer_InterruptFlagClear(ADVANCED_TIMER3, ADV_TIMER_TIM_SR_COMIF);
    }
...
void TIMER_Config(void)
{
    ...
    Timer_DmaEnable(ADVANCED_TIMER3, ADV_TIMER_TIM_DIER_COMIE | ADV_TIMER_TIM_DIER_COMDE);
    ...
    Timer_ChannelControlShadowUpdateConfig(ADVANCED_TIMER3,ENABLE); 
    Timer_ChannelOutputModeConfig(ADVANCED_TIMER3, TIMER_CH_1,  ADV_TIMER_TIM_CCMR1_OC1M_PWM1); 
    Timer_ChannelControlShadowConfig(ADVANCED_TIMER3, ENABLE);      

    Timer_DmaTransferConfig(ADVANCED_TIMER3,0x0D,0);
    ...
    delay_1ms(2);
    Timer_ChannelOutputModeConfig(ADVANCED_TIMER3, TIMER_CH_1,  ADV_TIMER_TIM_CCMR1_OC1M_PWM2); 
    delay_1ms(2);
    Timer_EventSoftwareGenerate(ADVANCED_TIMER3,ADV_TIMER_TIM_EGR_COMG);
}
...
void main(void)
{
    ...
    UDMA_PAM2MStruct.UDMA_DstBaseAddr = CAL_ADDR_LO(ADVANCED_TIMER3_CH0CV_ADDR);
    UDMA_PAM2MStruct.UDMA_SrcBaseAddr = CAL_ADDR_LO((src_base_addr));
#if defined(SOC_ADDRESS_HIGH_VALID) && SOC_ADDRESS_HIGH_VALID == 1
    UDMA_PAM2MStruct.UDMA_DstBaseAddr_H = CAL_ADDR_HI(ADVANCED_TIMER3_CH0CV_ADDR);
    ...
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_ADVANCED_TIMER3_COM;
    UDMA_PAM2M_Init(ADVANCED_TIMER3_COM_DMA_CH, &UDMA_PAM2MStruct);

    ECLIC_Register_IRQ(UDMA0_IRQn, ECLIC_NON_VECTOR_INTERRUPT,
                    ECLIC_LEVEL_TRIGGER, 1, 0,
                    UDMA0_IRQHandler);

    UDMA_PA2M_ITConfig(ADVANCED_TIMER3_COM_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
    ...
    if((state0==0)&(state1==0)&(ADVANCED_TIMER3->TIM_CCR1==250)){
        simulation_pass();
    }else{
        simulation_fail();
    }
