...
void TIMER_Config(void)
{
    ...
    Timer_DmaEnable(ADVANCED_TIMER3, ADV_TIMER_TIM_DIER_CC1DE_ENABLE);
    ...
    Timer_ChannelDmaRequestSourceSelect(ADVANCED_TIMER3,ADV_TIMER_TIM_CR2_CCDS_ENABLE);
...
{
     if (UDMA_PA2M_GetITStatus(ADVANCED_TIMER3_CC1_DMA_IRQ, PA2M_FTRANS_IRQ_STAT) == SET) {
        Timer_DmaEnable(ADVANCED_TIMER3, ADV_TIMER_TIM_DIER_CC1DE_DISABLE);
        state=0;
        UDMA_PA2M_ClearITStatus(ADVANCED_TIMER3_CC1_DMA_IRQ, PA2M_FTRANS_IRQ_CLEAR_STAT);
     }
}
...
void main(void)
{
    ...
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_ADVANCED_TIMER3_CC1;
    UDMA_PAM2M_Init(ADVANCED_TIMER3_CC1_DMA_CH, &UDMA_PAM2MStruct);

    ECLIC_Register_IRQ(UDMA0_IRQn, ECLIC_NON_VECTOR_INTERRUPT,
                    ECLIC_LEVEL_TRIGGER, 1, 0,
                    UDMA0_IRQHandler);

    UDMA_PA2M_ITConfig(ADVANCED_TIMER3_CC1_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
    ...
    if(ADVANCED_TIMER3->TIM_CCR1==250){
        simulation_pass();
    }else{
        simulation_fail();
    }
