...
uint16_t USART4_send_buffer[ARR_SIZE]={ 0x101 ,0x102,0x103,0x104,0x105,0x106,0x107,0x108,0x109,0x10a};
...
void main(void)
{
    ...
    do {
        USART_Send9Bit(USART4, USART4_send_buffer[i]);
        USART4_receive_buffer[i] = USART_Receive9Bit(USART4);
    } while ((++i) < ARR_SIZE);
    state0 = MemoryCompare(USART4_send_buffer, USART4_receive_buffer, ARR_SIZE);
