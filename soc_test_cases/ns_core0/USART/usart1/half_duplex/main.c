...
int main(void)
{
    ...
    /* USART half duplex mode enable */
    USART_SimplexConfig(USART1, ENABLE);
    USART_SimplexConfig(USART2, ENABLE);
    ...
    /* USART2 tx enable rx disable */
    usart_init_t.USART_TX_CTL = USART_TXCTRL_TXEN_ENABLE;
    usart_init_t.USART_RX_CTL = USART_RXCTRL_RXEN_DISABLE;
    USART_Init(USART2, &usart_init_t);
    /* USART1 rx enable tx disable */
    usart_init_t.USART_TX_CTL = USART_TXCTRL_TXEN_DISABLE;
    usart_init_t.USART_RX_CTL = USART_RXCTRL_RXEN_ENABLE;
    USART_Init(USART1,&usart_init_t);
