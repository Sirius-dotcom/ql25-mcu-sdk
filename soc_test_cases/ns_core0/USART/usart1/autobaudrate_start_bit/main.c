...
uint8_t USART2_send_buffer[ARR_SIZE]={ 0x55 ,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1a};
...
static void AutoBauRate_StartBitMethod(void)
{
  ...
  while(USART_GetABRStatus(USART1, USART_ADVANCED_STATUS_ABRF) == RESET)
  {}
 
  /* If AutoBaudBate error occurred */
  if (USART_GetABRStatus(USART1, USART_ADVANCED_STATUS_ABRE) != RESET)
  {
  ...
  else
  {
            ...
            baudrateval= usart_init_t.USART_CLK_Feq /((USART1->DIV) +1) ;
    #else
            baudrateval= SystemCoreClock /((USART1->DIV) +1) ;
    #endif
...
void main(void )
{
    ...
    usart_init_t.AutoBaudRateMode = USART_ADVANCED_SETUP_ABR_MOD_START_BIT; 
    usart_init_t.USART_Fra_Baud = 0;
    /* USART_StructInit(&usart_init_t) */
    ...
    for(uint8_t i;i<10;i++){
        USART_SendData(USART2, USART2_send_buffer[i]);
    }
