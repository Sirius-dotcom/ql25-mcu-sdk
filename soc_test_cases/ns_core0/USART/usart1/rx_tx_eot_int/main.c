...
{
    if(USART_GetITStatus(USART1, USART_STATUS_RX_EOT|USART_STATUS_TX_EOT)==SET)
    {
        USART_ClearITStatus(USART1, USART_STATUS_RX_EOT|USART_STATUS_TX_EOT);
        state0=0;
    }
}
...
void main(void )
{
    ...
    USART_Set_RXSize(USART1,3);
    USART_Set_TXSize(USART1,3);
    /* USART1 overflow irq enable */
    USART_ITConfig(USART1, USART_INT_EN_RX_EOT_IRQ_EN |USART_INT_EN_TX_EOT_IRQ_EN, ENABLE);
    int i = 0;
    do {
        USART_SendData(USART1, USART1_send_buffer[i]);
        USART1_receive_buffer[i] = USART_ReceiveData(USART1);
    } while ((++i) < ARR_SIZE);
