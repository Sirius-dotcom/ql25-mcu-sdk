...
#define ARR_SIZE 8     
uint8_t USART1_send_buffer[ARR_SIZE]={0x01,0x02,0x03,0x01,0x02,0x03,0x01,0x02};
...
{       
    if(USART_GetITStatus(USART2,USART_STATUS_EMPTY) == SET)
        {
            USART_SendArray(USART2,temp1,ARR_SIZE);
            USART_ITConfig(USART2,USART_INT_EN_RX_FIFO_EMPTY_IRQ_EN,DISABLE);
            state1 = 0;
        }
...
void main(void)
{
    ...
    USART_ITConfig(USART2,USART_INT_EN_RX_FIFO_EMPTY_IRQ_EN,ENABLE);
