...
#define ARR_SIZE 8     
uint8_t USART3_send_buffer[ARR_SIZE]={0x01,0x02,0x03,0x01,0x02,0x03,0x01,0x02};
...
{       
    if(USART_GetITStatus(USART4,USART_STATUS_EMPTY) == SET)
        {
            USART_SendArray(USART4,temp1,ARR_SIZE);
            USART_ITConfig(USART4,USART_INT_EN_RX_FIFO_EMPTY_IRQ_EN,DISABLE);
            state1 = 0;
        }
...
void main(void)
{
    ...
    USART_ITConfig(USART4,USART_INT_EN_RX_FIFO_EMPTY_IRQ_EN,ENABLE);
