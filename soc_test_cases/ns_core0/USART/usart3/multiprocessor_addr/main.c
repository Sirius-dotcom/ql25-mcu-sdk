...
uint16_t USART3_send_buffer[ARR_SIZE]={ 0x102 ,0x11,0x22,0x12,0x13,0x14,0x15,0x16,0x17};
uint16_t USART4_receive_buffer[ARR_SIZE]={0};
uint16_t USART3_receive_buffer[ARR_SIZE]={};
uint16_t USART4_send_buffer[ARR_SIZE]={ 0x11 ,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1a};
...
void main(void )
{
    ...
    USART_WakeUpConfig(USART4,USART_ADVANCED_SETUP_WAKE_MOD_ADDR);
    
    USART_ReceiverWakeUpCmd(USART4, ENABLE);
    
    USART_WriteWakeUp(USART4);
    
    USART_SetRXAddress(USART4,USART_ADVANCED_SETUP_ADD7_7_BIT, 0x02);
    ...
    while(USART_GetRXCTRL_Status(USART4, USART_RXCTRL_CLR)){}
    for(uint16_t i=0;i<10;i++){
        USART_Send9Bit(USART3, USART3_send_buffer[i]);
    }
    delay_1ms(4);
    
    if((USART_ADVANCED_STATUS_RWUFMM!=USART_READWakeUp(USART4)))
    {
        if(USART_Receive9Bit(USART4)==0x102){
            if(USART_Receive9Bit(USART4)==0x11){
                if(USART_Receive9Bit(USART4)==0x22){
                    simulation_pass();
                }else {
                    simulation_fail();
                }
