...
uint8_t USART3_send_buffer[ARR_SIZE]={0x01,0x02,0x13,0x14,0x05,0x08,0x06,0x1a,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x01,0x02,0x13,0x14,0x05,0x08,0x06,0x1a,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08};
uint8_t USART4_receive_buffer[ARR_SIZE] = {};
...
void USART_ReceiveArray(USART_TypeDef *USARTx,int length)
{
    ...
    for(int j=0;j<length;j++){
        ...
        USART4_receive_buffer[j] = USART_ReceiveData(USARTx);
    }
}
...
{
    if(USART_GetITStatus(USART3,USART_STATUS_FULL) == SET)
    {
        ...
        USART_ClearITStatus(USART3,USART_STATUS_FULL);
        USART_ITConfig(USART3,USART_INT_EN_TX_FIFO_FULL_IRQ_EN,DISABLE);
        state1=0;
        state0=1;
...
void main(void)
{
    ...
    usart_init_t.USART_RX_Data_Size = USART_RX_DATASIZE_NUM_BYTE;
    USART_Init(USART3, &usart_init_t);
    /* USART4 transmit open */
    usart_init_t.USART_TX_CTL = USART_TXCTRL_TXEN_ENABLE;
    USART_Init(USART4, &usart_init_t);

    USART_ITConfig(USART3,USART_INT_EN_TX_FIFO_FULL_IRQ_EN,ENABLE);

    USART_CleanFifo_RX(USART4);
    USART_CleanFifo_TX(USART3);

    /* write data to USART3 tx fifo */
