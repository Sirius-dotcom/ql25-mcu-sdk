...
#define SRC_DATA_ACC_ADDR_BASE1          (0x8000000 + 32768 - 0x300)
#define DES_DATA_ACC_ADDR_BASE1          (0x8000000 + 32768 - 0x400)
...
int main(void)
{
    ...
    memcpy((uint8_t *)src_base_addr, (uint8_t *)USART3_send_buffer, ARR_SIZE);
    ...
    memcpy((uint8_t *)src_base_addr1, (uint8_t *)USART4_send_buffer, ARR_SIZE);

    UDMA_PA2M_ITConfig(USART4_USART_DMA_RX_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
    UDMA_PA2M_ITConfig(USART3_USART_DMA_RX_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
    ...
    UDMA_PAM2MStruct.UDMA_SrcBaseAddr = CAL_ADDR_LO(&(USART3->RXDATA));

#if defined(SOC_ADDRESS_HIGH_VALID) && SOC_ADDRESS_HIGH_VALID == 1
    UDMA_PAM2MStruct.UDMA_DstBaseAddr_H = CAL_ADDR_HI(dst_base_addr1);
    UDMA_PAM2MStruct.UDMA_SrcBaseAddr_H = CAL_ADDR_HI(&(USART3->RXDATA));
#endif
    ...
    UDMA_PAM2MStruct.UDMA_Width = PA2M_MDWIDTH_8BIT;
    UDMA_PAM2MStruct.UDMA_Mode = PA2M_MODE_NORMAL;
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_USART3_USART_DMA_RX;  
    UDMA_PAM2M_Init(USART3_USART_DMA_RX_DMA_CH, &UDMA_PAM2MStruct);

    UDMA_PAM2MStruct.UDMA_TransEn = PA2M_TRANS_ENABLE;
    UDMA_PAM2MStruct.UDMA_DstBaseAddr = CAL_ADDR_LO(&(USART4->TXDATA));
    UDMA_PAM2MStruct.UDMA_SrcBaseAddr = CAL_ADDR_LO((src_base_addr1));

#if defined(SOC_ADDRESS_HIGH_VALID) && SOC_ADDRESS_HIGH_VALID == 1
    UDMA_PAM2MStruct.UDMA_DstBaseAddr_H = CAL_ADDR_HI(&(USART4->TXDATA));
    ...
    UDMA_PAM2MStruct.UDMA_Width = PA2M_MDWIDTH_8BIT;
    UDMA_PAM2MStruct.UDMA_Mode = PA2M_MODE_NORMAL;
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_USART4_USART_DMA_TX;
    UDMA_PAM2M_Init(USART4_USART_DMA_TX_DMA_CH, &UDMA_PAM2MStruct);

    while (UDMA_PA2M_GetITStatus(USART4_USART_DMA_RX_DMA_IRQ, PA2M_FTRANS_IRQ_STAT) == 0){};
    while (UDMA_PA2M_GetITStatus(USART3_USART_DMA_RX_DMA_IRQ, PA2M_FTRANS_IRQ_STAT) == 0){};
    memcpy((uint8_t *)USART4_receive_buffer, (uint8_t *)dst_base_addr, ARR_SIZE );
    memcpy((uint8_t *)USART3_receive_buffer, (uint8_t *)dst_base_addr1, ARR_SIZE );

    state0 = MemoryCompare((uint8_t*)USART3_send_buffer, (uint8_t*)USART4_receive_buffer, ARR_SIZE);
    state1 = MemoryCompare(USART4_send_buffer, USART3_receive_buffer, ARR_SIZE);
    if (state0 & state1) {
        simulation_pass();
    } else {
        simulation_fail();
