...
{
    iomux_ls_iof_oval_cfg(IOMUX_BASE,USART3_SCLK_IOF_OVAL,USART3_SCLK_PAD_SEL,USART3_SCLK_HS_SEL,0,0);
...
void iomux_config(void)
{
    iomux_ls_iof_oval_cfg(IOMUX_BASE,USART3_TX_CLK_IOF_OVAL,USART3_TX_CLK_PAD_SEL,USART3_TX_CLK_HS_SEL,0,0);
    iomux_ls_iof_oval_cfg(IOMUX_BASE,USART3_TX_IOF_OVAL,USART3_TX_PAD_SEL,USART3_TX_HS_SEL,0,0);
    iomux_ls_iof_ival_cfg(IOMUX_BASE,USART3_RX_IOF_IVAL,USART3_RX_PAD_SEL,USART3_RX_HS_SEL,0,0);
    #ifdef QSPI3_SCK_IOF_IVAL
    iomux_ls_iof_ival_cfg(IOMUX_BASE, QSPI3_SCK_IOF_IVAL, QSPI3_SCK_PAD_SEL, QSPI3_SCK_HS_SEL, 0, 0);
...
int main(void)
{
    ...
    while (data_read2--) {
        ...
        /* write one byte in the USART3 data register */
        USART_SendData(USART3, txbuffer1[tx_counter1++]);
        /* wait until end of transmit */
        while (SET == USART_GetFlag(USART3, USART_STATUS_TX_BUSY)) {}
    ...
    USART_Init(USART3, &usart_init_t);
    /* clear the USART3 data register */
    USART_CleanFifo_RX(USART3);
    USART_CleanFifo_TX(USART3);
    ...
    while (data_read1--) {
        ...
        while (SET == USART_GetFlag(USART3, USART_STATUS_RX_BUSY)) {}
        /* store the received byte in the rxbuffer1 */
        rxbuffer1[rx_counter1++] = USART_ReceiveData(USART3);
        QSPI_SSMSwitch(QSPI3, DISABLE);
    }
