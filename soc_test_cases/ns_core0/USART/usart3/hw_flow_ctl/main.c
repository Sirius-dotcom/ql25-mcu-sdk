...
uint8_t USART3_send_buffer[ARR_SIZE]={ 0xa1 ,0xa2,0xa3,0xa4,0xa5,0xa6,0xa7,0xa8,0xa9,0xaa};
...
void main(void)
{
    ...
    do {
        USART_SendData(USART3, USART3_send_buffer[send_i]);
        send_i++;
    } while((send_i) < ARR_SIZE);

    while (flag) {}
    do {
        USART4_receive_buffer[rece_i] = USART_ReceiveData(USART4);
        rece_i++;
    ...
    USART_ITConfig(USART3, (USART_INT_EN_CTSIE_RISE_EN | USART_INT_EN_CTSIE_FALL_EN), DISABLE);
    USART_CleanFifo_RX(USART4);
    USART_CleanFifo_TX(USART3);
    while(USART_GetTXCTRL_Status(USART3, USART_TXCTRL_CLR)){}
