...
/**
  ...
  */
void BASIC_TIMER4_IRQHandler(void)
{
    static uint8_t num = 0;
    if(SET == Basic_Timer_InterruptFlagGet(BASIC_TIMER4, BASIC_TIMER_DIER_UIE)) {
        /* clear channel 0 interrupt bit */
        Basic_Timer_InterruptFlagClear(BASIC_TIMER4, BASIC_TIMER_DIER_UIE);
        num++;
        if (3 <= num)
        {
            state = 0;
            Basic_Timer_Disable(BASIC_TIMER4);
        }
    }
}
...
int main(void)
{
    ...
    ECLIC_Register_IRQ(BASIC_TIMER4_IRQn, ECLIC_NON_VECTOR_INTERRUPT,
                                    ECLIC_LEVEL_TRIGGER, 1, 0,
                                    BASIC_TIMER4_IRQHandler);
    UDMA_PAM2MTypeDef UDMA_PAM2MStruct = {0};
    UDMA_PAM2MStruct.UDMA_TransEn = PA2M_TRANS_ENABLE;
    UDMA_PAM2MStruct.UDMA_DstBaseAddr = CAL_ADDR_LO(BASIC_TIMER4 + 0x2C);
    ...
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_BASIC_TIMER4_I;
    UDMA_PAM2M_Init(BASIC_TIMER4_I_DMA_CH, &UDMA_PAM2MStruct);

    Basic_TIMER_Init_TypeDef timer_initpara;
    /* initialize TIMER init parameter struct */
    Basic_Timer_StructParaInit(&timer_initpara);
    /* BASIC_TIMER4 configuration */
    ...
    Basic_Timer_Init(BASIC_TIMER4, &timer_initpara);
    Basic_Timer_DmaEnable(BASIC_TIMER4, BASIC_TIMER_DIER_UDE);
    Basic_Timer_InterruptEnable(BASIC_TIMER4, BASIC_TIMER_DIER_UIE);
    Basic_Timer_Enable(BASIC_TIMER4);
