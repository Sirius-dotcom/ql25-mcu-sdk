...
void main(void)
{
    ...
    /* repeat mode, address not accumulation check */
    for (int i = 0; i < UDMA0_TRANSFER_BYTE_NUMS; i++) {
        if (((uint8_t*)(CAL_ADDR(SRC_DATA_ACC_ADDR_BASE)))[i] != ((uint8_t*)(CAL_ADDR(DES_DATA_ACC_ADDR_BASE + 3 * UDMA0_TRANSFER_BYTE_NUMS)))[i]) {
            cmp_status = 1;
        }
    }
