...
#define SRC_DATA_ACC_ADDR_BASE           (0x9000000 + 32768 - 0x400)
#define DES_DATA_ACC_ADDR_BASE           (0x9000000 + 32768 - 0x200)

static uint8_t __attribute__ ((aligned (4))) src_buffer_8[256] = {0x12,0x5a,0xa5,0xaa,0x55,0xa5,0x12,0x56,0x34,0x78,0x12,0x78,0x12,0x78,0x34,0x78,
                            ...
                            0x12,0x5a,0xa5,0xaa,0x55,0xa5,0x12,0x56,0x34,0x78,0x12,0x78,0x12,0x78,0x34,0x78,
                            0x12,0x5a,0xa5,0xaa,0x55,0xa5,0x12,0x56,0x34,0x78,0x12,0x78,0x12,0x78,0x34,0x78,};
static uint8_t __attribute__ ((aligned (4))) dst_buffer_8[256] = {0};
...
void UDMA0_IRQHandler(void)
{
    if (UDMA_GetITStatus(UDMA0_CH0_M2M_IRQ, UDMA_FTRANS_IRQ) == SET) {
        ...
        intCnt++;
        state2 = 1;
        UDMA_ITConfig(UDMA0_CH0_M2M_IRQ, UDMA_HTRANS_IRQ, DISABLE);
...
void main(void)
{
    ...
    UDMA_RptConfig(UDMA0_M2M_CH0, UDMA0_REPEAT_TRAN_NUMS, UDMA_RPT_SAUM_SUB_MODE, UDMA0_TRANSFER_BYTE_NUMS, UDMA_RPT_DAUM_SUB_MODE, 0);
    ...
    if (intCnt > 2) {
        printf("Trigger multi-time IRQ: %d\r\n", intCnt);
        delay_1ms(1);
        simulation_fail();
        while (1);
    }
    for (int i = 0; i < UDMA0_TRANSFER_BYTE_NUMS; i++) {
        if (((uint8_t*)(CAL_ADDR(SRC_DATA_ACC_ADDR_BASE)))[i] != ((uint8_t*)(CAL_ADDR(DES_DATA_ACC_ADDR_BASE + (UDMA0_REPEAT_TRAN_NUMS - 1) * UDMA0_TRANSFER_BYTE_NUMS)))[i]) {
            printf("des[%d]: %x\r\n", i, ((uint8_t*)(CAL_ADDR(DES_DATA_ACC_ADDR_BASE + (UDMA0_REPEAT_TRAN_NUMS - 1) * UDMA0_TRANSFER_BYTE_NUMS)))[i]);
            simulation_fail();
            while (1);
        }
    }
