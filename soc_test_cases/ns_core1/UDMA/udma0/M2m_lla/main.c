...
void main(void)
{
    ...
    s_lla_cfg->MCTRL = (UDMA_TRANS_ENABLE | UDMA_MODE_NORMAL | UDMA_PRIORITY_HIGH | UDMA_MDNA_ENABLE | UDMA_MSNA_ENABLE | UDMA_MSWIDTH_32BIT | UDMA_MDWIDTH_32BIT);
    s_lla_cfg->RPT = 0;
    ...
    if ((ftrans_flg > 1) || (lla_ftrans_flg > 1)) {
        simulation_fail();
    } else {
        simulation_pass();
    }
