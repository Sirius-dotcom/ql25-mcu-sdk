...
void TIMER_Config(void)
{
    ...
    Timer_DmaEnable(ADVANCED_TIMER0, ADV_TIMER_TIM_DIER_UDE_ENABLE);
    Timer_DmaTransferConfig(ADVANCED_TIMER0,0x0D,2);
    Timer_Init(ADVANCED_TIMER0, &timer_initpara);
    Timer_PrescalerConfig(ADVANCED_TIMER0, ADV_TIMER_TIM_EGR_UG);
}
...
{
     if (UDMA_PA2M_GetITStatus(ADVANCED_TIMER0_UEV_DMA_IRQ, PA2M_FTRANS_IRQ_STAT) == SET) {
        Timer_DmaEnable(ADVANCED_TIMER0, ADV_TIMER_TIM_DIER_UDE_DISABLE);
        state=0;
        UDMA_PA2M_ClearITStatus(ADVANCED_TIMER0_UEV_DMA_IRQ, PA2M_FTRANS_IRQ_CLEAR_STAT);
     }
}
...
void main(void)
{
...
#endif
    UDMA_PAM2MStruct.UDMA_BufferSize = 4*3;
    ...
    UDMA_PAM2MStruct.UDMA_PER_SEL = UDMA_SEL_ADVANCED_TIMER0_UEV;
    UDMA_PAM2M_Init(ADVANCED_TIMER0_UEV_DMA_CH, &UDMA_PAM2MStruct);

    ECLIC_Register_IRQ(UDMA0_IRQn, ECLIC_NON_VECTOR_INTERRUPT,
                    ECLIC_LEVEL_TRIGGER, 1, 0,
                    UDMA0_IRQHandler);

    UDMA_PA2M_ITConfig(ADVANCED_TIMER0_UEV_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
    ...
    if((ADVANCED_TIMER0->TIM_CCR1==250)&(ADVANCED_TIMER0->TIM_CCR2==500)&(ADVANCED_TIMER0->TIM_CCR3==750)){
        simulation_pass();
    }else{
        simulation_fail();
    }
