...
void ADVANCED_TIMER3_IRQHandler(void)
{
    ...
    if(Timer_FlagGet(ADVANCED_TIMER3, ADV_TIMER_TIM_DIER_UIE) == SET)
    {
        if((ADVANCED_TIMER3->TIM_CR1 & ADV_TIMER_TIM_CR1_DIR)== ADV_TIMER_TIM_CR1_DIR)
        {
            cnt_z++;
        }else{
            cnt_f++;
        ...
        Timer_InterruptFlagClear(ADVANCED_TIMER3, ADV_TIMER_TIM_SR_UIF);
...
void TIMER_Config(void)
{
    ...
    Timer_SlaveModeSelect(ADVANCED_TIMER3,  ADV_TIMER_TIM_SMCR_SMS_ENCODER3);
    ...
    Timer_InterruptEnable(ADVANCED_TIMER3, ADV_TIMER_TIM_DIER_UIE); 
