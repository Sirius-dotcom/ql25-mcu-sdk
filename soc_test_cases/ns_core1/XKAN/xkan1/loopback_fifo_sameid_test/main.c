...
void xkan_config(void)
{
    ...
    */
    ...
    xKan_ConfigRxFifoWatermark(XKAN1, XKAN_CFG_RX_FIFO0, FRAMENUM-1);
}
...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    _DataBuf = tmp_msg5;
    xKan_AddMessageToTxFifo(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN1_RAM_BASE);
    ...
    xKan_EnableTxBufferRequest(XKAN1, XKAN_TX_BUFFER0|XKAN_TX_BUFFER1|XKAN_TX_BUFFER2|XKAN_TX_BUFFER3|XKAN_TX_BUFFER4);
}
...
void XKAN1_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_RXF0_WM_INT) && xKan_GetInterruptEnable(XKAN1, XKAN_CFG_INT_EN_RXF0_WM_EN))
    {
        for(; rxfifo0_done < FRAMENUM; rxfifo0_done ++){
            ...
            pRxData += BUFSIZE;
            if (rxfifo0_done == FRAMENUM) xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_RXF0_WM_EN, DISABLE);
        }
    }
...
void main(void)
{
    ...
    xKan_EnableKernel(XKAN1, ENABLE);
