...
uint8_t USART2_receive_buffer2[ARR_SIZE] = {0};
uint8_t state0 = 0;
...
void UDMA0_IRQHandler(void)
{
    ...
    if (UDMA_PA2M_GetITStatus(USART2_USART_DMA_RX_DMA_IRQ, PA2M_FTRANS_IRQ_STAT)) {
        ...
        UDMA_PA2M_ClearITStatus(USART2_USART_DMA_RX_DMA_IRQ, PA2M_FTRANS_IRQ_CLEAR_STAT);
        interrupt_time_rx ++;
    }
}
...
int main(void)
{
    ...
    /*usart clean fifo*/
    while (USART_GetFlag(USART1, USART_STATUS_TX_BUSY)) {}
    while (USART_GetFlag(USART2, USART_STATUS_RX_BUSY)) {}
