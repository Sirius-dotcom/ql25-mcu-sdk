...
uint16_t USART1_send_buffer[ARR_SIZE]={ 0x101 ,0x102,0x103,0x104,0x105,0x106,0x107,0x108,0x109,0x10a};
uint16_t USART2_receive_buffer[ARR_SIZE]={0};
uint16_t USART1_receive_buffer[ARR_SIZE]={0};
uint16_t USART2_send_buffer[ARR_SIZE]={ 0x1F6 ,0x1F7,0x1F8,0x1F9,0x1FA,0x1FB,0x1FC,0x1FD,0x1FE,0x1FF};
...
void main(void )
{
    ...
    do {
        USART_Send9Bit(USART2, USART2_send_buffer[i]);
        USART1_receive_buffer[i] = USART_Receive9Bit(USART1);
    } while ((++i) < ARR_SIZE);
    USART_CleanFifo_RX(USART2);
    USART_CleanFifo_TX(USART2);
    
    do {
        USART_Send9Bit(USART1, USART1_send_buffer[j]);
        USART2_receive_buffer[j] = USART_Receive9Bit(USART2);
    } while ((++j) < ARR_SIZE);
