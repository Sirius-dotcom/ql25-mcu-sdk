...
{
    if (USART_GetITStatus(USART4, USART_STATUS_RX_IDLE_TOUT_STATUS))
    {

        if(state0==1){
            USART_ClearITStatus(USART4, USART_STATUS_RX_IDLE_TOUT_STATUS);
            USART_ITConfig(USART4, USART_INT_EN_RX_IDLE_TOUT_IRQ_EN, DISABLE);
        }
        if(state0==0){
            USART4_receive_buffer[0] = USART_ReceiveData(USART4);
...
void main(void)
{
    ...
    /* USART4 idle irq enable */
    USART_ITConfig(USART4, USART_INT_EN_RX_IDLE_TOUT_IRQ_EN, ENABLE);
