...
uint8_t USART4_receive_buffer2[ARR_SIZE] = {0};
uint8_t state0 = 0;
uint8_t interrupt_time_tx = 0;
uint8_t interrupt_time_rx = 0;
...
{
    if (UDMA_PA2M_GetITStatus(USART3_USART_DMA_TX_DMA_IRQ, PA2M_FTRANS_IRQ_STAT)) {
        ...
        UDMA_PA2M_ClearITStatus(USART3_USART_DMA_TX_DMA_IRQ, PA2M_FTRANS_IRQ_CLEAR_STAT);
        interrupt_time_tx ++;
    }
...
int main(void)
{
    ...
    UDMA_PA2M_ITConfig(USART3_USART_DMA_TX_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
    ...
    usart_init_t.USART_DMA_EN = USART_SETUP_DMA_EN_DISABLE;
    ...
    /* Wait for some time to complete at least one complete transmission */
    /*usart clean fifo*/
    while (USART_GetFlag(USART3, USART_STATUS_TX_BUSY)) {}
    while (USART_GetFlag(USART4, USART_STATUS_RX_BUSY)) {}
    while(interrupt_time_tx<3){}
    while(interrupt_time_rx<3){}
