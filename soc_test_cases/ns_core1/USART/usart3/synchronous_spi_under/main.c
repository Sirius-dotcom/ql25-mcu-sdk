...
#ifdef USART3_SCLK_PAD_SEL

void iomux_config(void)
{
    iomux_ls_iof_ival_cfg(IOMUX_BASE,USART3_SCLK_IOF_IVAL,USART3_SCLK_PAD_SEL,USART3_SCLK_HS_SEL,0,0);
...
void iomux_config(void)
{
    iomux_ls_iof_ival_cfg(IOMUX_BASE,USART3_TX_CLK_IOF_IVAL,USART3_TX_CLK_PAD_SEL,USART3_TX_CLK_HS_SEL,0,0);
    iomux_ls_iof_oval_cfg(IOMUX_BASE,USART3_TX_IOF_OVAL,USART3_TX_PAD_SEL,USART3_TX_HS_SEL,0,0);
    iomux_ls_iof_ival_cfg(IOMUX_BASE,USART3_RX_IOF_IVAL,USART3_RX_PAD_SEL,USART3_RX_HS_SEL,0,0);
    #ifdef QSPI3_SCK_IOF_IVAL
    iomux_ls_iof_oval_cfg(IOMUX_BASE, QSPI3_SCK_IOF_OVAL, QSPI3_SCK_PAD_SEL, QSPI3_SCK_HS_SEL, 0, 0);
...
void USART3_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART3,USART_STATUS_SPI_SLV_UNDER_RUN) == SET)
    {
        USART_ClearITStatus(USART3,USART_STATUS_SPI_SLV_UNDER_RUN);
        USART_ITConfig(USART3,USART_INT_EN_SPI_SLV_UNDER_RUN_IRQ_EN,DISABLE);
        state3 =0;
        state0 =1;
    }
...
int main(void)
{
    ...
    USART_SynClockConfig(USART3, ENABLE);
    USART_SLV_EN(USART3,ENABLE);
    USART_DIS_NSS_EN(USART3,ENABLE);
    USART_CleanFifo_RX(USART3);
    USART_CleanFifo_TX(USART3);
    USART_ITConfig(USART3,USART_INT_EN_SPI_SLV_UNDER_RUN_IRQ_EN,ENABLE);

    while (data_read2--) {
        /* send a dummy byte to generate clock to slave */
        QSPI_SendData(QSPI3, 0xff);
