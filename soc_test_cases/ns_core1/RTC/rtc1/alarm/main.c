...
#define HART_NUM 2
#define RTC_AS_EVENT ((RTC1_IRQn - 20) % 32)
#define IRQ_AS_EVENT_OFS 0x180
#define EVENT_SEL_OFS 0x100
#define EVENT_ENA_ADDR (IDU_BASE + IRQ_AS_EVENT_OFS + ((RTC1_IRQn - 20) / 32) * 4)
...
{
    RTC_CounerSet(RTC1, 0);
    RTC_AlarmSet(RTC1, 0x3);
    /* set RTC prescaler: set RTC period to 1s */
    RTC_PrescalerSet(RTC1, 0xF);

    /* enable the RTC alarm interrupt */
    RTC_InterruptEnable(RTC1, RTC_CRH_ALRIE);
}
