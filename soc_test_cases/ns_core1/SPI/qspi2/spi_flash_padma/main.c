...
void SPI_PageWrite_Dma(uint8_t cmd, uint32_t addr)
{
    ...
    while (!UDMA_PA2M_GetITStatus(QSPI2_TX_DMA_DMA_IRQ, PA2M_FTRANS_IRQ_STAT)) {}
    while (QSPI_GetFlag(QSPI2,QSPI_STATUS_BUSY) == SET) {}
    Spi_NorFlash_CS(QSPI2, CS_OFF);
    QSPI_TxDmaStart(QSPI2, DISABLE);
...
void SPI_PageRead_Dma(uint8_t cmd, uint32_t addr)
{
    ...
    QSPI_BusWidthSwitch(QSPI2, bus_width);
...
void main(void)
{
    ...
    UDMA_PA2M_ITConfig(QSPI2_TX_DMA_DMA_IRQ, PA2M_FTRANS_IRQ_EN, ENABLE);
