...
void SPI_WriteByte(uint8_t txdata)
{
    ...
    QSPI_SendData(QSPI1, txdata);
    while (SET == QSPI_GetFlag(QSPI1, QSPI_STATUS_BUSY)) {}
}
...
uint8_t SPI_ReadByte(void)
{
    ...
    QSPI_SendData(QSPI1, Dummy_Byte);
    while (SET == QSPI_GetFlag(QSPI1, QSPI_STATUS_RX_EMPTY)) {}
    rxdata = QSPI_ReceiveData(QSPI1) & 0xFF;
    return rxdata;
}
...
void SPI_PageWrite(uint8_t cmd, uint8_t* buf, uint32_t addr)
{
    ...
    if(PSRAM_QUAD_WRITE == cmd) {
        QSPI1->FMT = value | QSPI_FMT_PROTO_QUAD;
    } else if(PSRAM_WRITE == cmd) {
        QSPI1->FMT = value | QSPI_FMT_PROTO_SINGLE;
    }
    SPI_WriteByte(cmd);
...
void main(void)
{
    ...
    QSPI1->FORCE &= ~(QSPI_FORCE_EN | QSPI_FORCE_WP);
