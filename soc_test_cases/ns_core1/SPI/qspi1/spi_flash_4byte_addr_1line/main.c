...
extern uint16_t flash_addr_mode;
...
void main(void)
{
    ...
    /* adjust flash address mode */
    flash_addr_mode = BYTE_MODE4;

    SPI_Init();

    Spi_NorFlash_ReadJEDECID(QSPI1);

    Spi_NorFlash_Enter4ByteMode(QSPI1);
    Spi_NorFlash_Erase_Sector(QSPI1, FLASH_ADDR);
    Spi_NorFlash_Write_Page(QSPI1, FLASH_PageProgram, write_buf, FLASH_ADDR, FLASH_PAGESIZE);
    Spi_NorFlash_Read(QSPI1, FLASH_FastReadData, read_buf, FLASH_ADDR, FLASH_PAGESIZE);
    Spi_NorFlash_Exit4ByteMode(QSPI1);

    if (0 == memcmp(write_buf, read_buf, FLASH_PAGESIZE)) {
        printf("4byte addr 1line mode data cmp pass\n");
    } else {
        printf("4byte addr 1line mode data cmp error\n");
        simulation_fail();
        printf("write_buf : ");
