...
void SPI_Init(void)
{
    ...
    /* QSPI3 parameter config */ /*Adjust the init sequence to prevent the "x" state */
...
void main(void)
{
    ...
    Spi_NorFlash_Write_Page(QSPI3, FLASH_PageProgram, write_buf, FLASH_ADDR, FLASH_PAGESIZE);

    Spi_NorFlash_Read(QSPI3, FLASH_DdrReadData, read_buf, FLASH_ADDR, FLASH_PAGESIZE);
    ...
    SPI_Init();
    memset(read_buf,0x00,FLASH_PAGESIZE);
    Spi_NorFlash_Read(QSPI3, FLASH_DdrReadDual, read_buf, FLASH_ADDR, FLASH_PAGESIZE);
    if (0 == memcmp(write_buf, read_buf, FLASH_PAGESIZE)) {
    ...
    Spi_NorFlash_Read(QSPI3, FLASH_DdrReadQuad, read_buf, FLASH_ADDR, FLASH_PAGESIZE);
