...
void main(void)
{
    ...
    printf("xkan normal_cancal_send_test \n\r");
    /* xkan cfg */ 
    xkan_config();
    ...
    state = xKan_AbortTxRequest(XKAN1, XKAN_TX_BUFFER1);
    state &= xKan_AbortTxRequest(XKAN1, XKAN_TX_BUFFER2);
    state &= xKan_AbortTxRequest(XKAN1, XKAN_TX_BUFFER3);

    /* wait for read over for rx_buffer */
    while(rx_done == 0){}

    /* compare the read buffer and write buffer */
    if(SUCCESS != MemoryCompare(g_KanTxData, g_KanRxData, BUFSIZE) || SUCCESS != state) {
        simulation_fail();
    } else {
        simulation_pass();
    }
