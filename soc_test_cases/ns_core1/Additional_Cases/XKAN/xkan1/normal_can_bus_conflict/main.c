...
uint8_t g_Kan2RxData[BUFSIZE*FRAMENUM];
uint8_t g_KanTxData[BUFSIZE*FRAMENUM];
uint8_t *pRxData = g_KanRxData;
uint8_t *pRxData2 = g_Kan2RxData;
uint8_t *pTxData = g_KanTxData;

volatile uint32_t rx1_done = 0;
volatile uint32_t rx2_done = 0;
uint8_t nop_cycle = 10;
...
void xkan_config(void)
{
    ...
    xkan_parameter_init.frame_format = XKAN_FRAME_FD_NO_BRS;
...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN2, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER0, XKAN2_RAM_BASE);
    ...
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN2, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1, XKAN2_RAM_BASE);
    ...
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN2, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER2, XKAN2_RAM_BASE);
    ...
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN2, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER3, XKAN2_RAM_BASE);

    xKan_EnableTxBufferRequest(XKAN1, XKAN_TX_BUFFER0|XKAN_TX_BUFFER1|XKAN_TX_BUFFER2|XKAN_TX_BUFFER3);
    while (nop_cycle--) {
        __NOP();
    }
    xKan_EnableTxBufferRequest(XKAN2, XKAN_TX_BUFFER0|XKAN_TX_BUFFER1|XKAN_TX_BUFFER2|XKAN_TX_BUFFER3);
}
...
void XKAN1_IRQHandler(void)
{
    ...
    {
        xKan_RxBufferNewMessageCallback(XKAN1, &xkan_parameter_init, &xKan_RxHeader, pRxData2, XKAN1_RAM_BASE);
        rx1_done ++;
        pRxData2 += BUFSIZE;
        if (rx1_done == 4 ) xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void XKAN2_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN2, XKAN_CFG_INT_STATUS_RXB_UPD_INT) && xKan_GetInterruptEnable(XKAN2, XKAN_CFG_INT_EN_RXB_UPD_EN))
    {
        ...
        rx2_done ++;
        pRxData += BUFSIZE;
        if (rx2_done == 4 ) xKan_EnableInterrupt(XKAN2, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void main(void)
{
    ...
    printf("xkan normal_can_bus_conflict test\n\r");
    /* xkan cfg */ 
    xkan_config();
    ...
    while((rx1_done != 4) || (rx2_done != 4) ){}

    /* compare the read buffer and write buffer */
    if(SUCCESS != MemoryCompare(g_KanTxData, g_KanRxData, BUFSIZE * FRAMENUM) || SUCCESS != MemoryCompare(g_KanTxData, g_Kan2RxData, BUFSIZE * FRAMENUM)) {
        simulation_fail();
    } else {
        simulation_pass();
    }
