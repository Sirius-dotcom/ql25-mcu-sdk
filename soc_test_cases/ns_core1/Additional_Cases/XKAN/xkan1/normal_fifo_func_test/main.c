...
void xkan_config(void)
{
    ...
    xkan_parameter_init.ram_info.RxFifo0ElmtsNbr = 5;
    xkan_parameter_init.ram_info.RxFifo0ElmtSize = XKAN_DATA_BYTES_8;
    xkan_parameter_init.ram_info.RxFifo1ElmtsNbr = 5;
    ...
    xkan_parameter_init.ram_info.TxBuffersNbr = 14;
    ...
    xKan_RXFilter.FilterID1 = 0x113;
    xKan_RXFilter.FilterID2 = 0x700;
    ...
    xKan_RXFilter.FilterID2 = 0x1FFFFF00;
    xKan_ConfigFilter(XKAN1, &xkan_parameter_init, &xKan_RXFilter, XKAN1_RAM_BASE);
    xKan_ConfigFilter(XKAN2, &xkan_parameter_init, &xKan_RXFilter, XKAN2_RAM_BASE);
    ...
    */
    ...
    xKan_ConfigGlobalFilter(XKAN2, XKAN_ACCEPT_IN_RX_FIFO0, XKAN_ACCEPT_IN_RX_FIFO1, XKAN_FILTER_REMOTE, XKAN_FILTER_REMOTE);

    /* step5: config rxfifo operation mode */
    xKan_ConfigRxFifoOperationMode(XKAN2, XKAN_CFG_RX_FIFO0, XKAN_RXFIFO_OP_BLOCK);
    xKan_ConfigRxFifoOperationMode(XKAN2, XKAN_CFG_RX_FIFO1, XKAN_RXFIFO_OP_OVERWRITE);
}
...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    for(int i = 0; i < FRAMENUM/2; i++) {
        xKan_TxHeader.Identifier = 0x177;
        xKan_TxHeader.IdType = XKAN_STANDARD_ID;
        _DataBuf = tmp_msg0;
        XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, (XKAN_TX_BUFFER0 << 2*i), XKAN1_RAM_BASE);

        xKan_TxHeader.Identifier = 0x12345687;
        xKan_TxHeader.IdType = XKAN_EXTENDED_ID;
        _DataBuf = tmp_msg1;
        XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1 << 2*i, XKAN1_RAM_BASE);
    }
    ...
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, (XKAN_TX_BUFFER0 << 2*5), XKAN1_RAM_BASE);

    xKan_TxHeader.Identifier = 0x12345687;
    xKan_TxHeader.IdType = XKAN_EXTENDED_ID;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1 << 2*5, XKAN1_RAM_BASE);

    /* miss frame */
    _DataBuf = tmp_msg3;
    xKan_TxHeader.Identifier = 0x277;
    xKan_TxHeader.IdType = XKAN_STANDARD_ID;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, (XKAN_TX_BUFFER0 << 2*6), XKAN1_RAM_BASE);

    xKan_TxHeader.Identifier = 0x13456789;
    xKan_TxHeader.IdType = XKAN_EXTENDED_ID;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1 << 2*6, XKAN1_RAM_BASE);

    xKan_EnableTxBufferRequest(XKAN1, 0x3FFF);
}
...
void main(void)
{
    ...
    printf("xkan normal_fifo_func_test\n\r");
    /* xkan cfg */ 
    xkan_config();
    ...
    for(int i = 0; i < BUFSIZE; i++){
        ...
        *(pTxData+BUFSIZE*(FRAMENUM-2)) = tmp_msg1[i];
        *(pTxData+BUFSIZE*(FRAMENUM-3)) = tmp_msg1[i];
        *(pTxData+BUFSIZE*(FRAMENUM-4)) = tmp_msg1[i];
        *(pTxData+BUFSIZE*(FRAMENUM-5)) = tmp_msg1[i];
        *(pTxData+BUFSIZE*(FRAMENUM-6)) = tmp_msg0[i];
        *(pTxData+BUFSIZE*(FRAMENUM-7)) = tmp_msg0[i];
        *(pTxData+BUFSIZE*(FRAMENUM-8)) = tmp_msg0[i];
        *(pTxData+BUFSIZE*(FRAMENUM-9)) = tmp_msg0[i];
        *pTxData ++ = tmp_msg2[i];
    }
    ...
    /* wait for tx over */
    while(XKAN1->CFG_TXBAR) {}
    while(XKAN1->CFG_TXREQ_PENDING) {}

    for(int i = 0; i < FRAMENUM/2; i++){
        xKan_RxFifo0Callback(XKAN2, &xkan_parameter_init, &xKan_RxHeader, pRxData, XKAN2_RAM_BASE);
        rxfifo0_done ++;
        pRxData += BUFSIZE;
    }

    for(int i = 0; i < FRAMENUM/2; i++){
        xKan_RxFifo1Callback(XKAN2, &xkan_parameter_init, &xKan_RxHeader, pRxData, XKAN2_RAM_BASE);
        rxfifo1_done ++;
        pRxData += BUFSIZE;
    }
