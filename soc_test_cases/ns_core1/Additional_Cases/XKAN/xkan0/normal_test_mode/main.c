...
volatile uint8_t XKAN1_rxbuf = 0;
volatile uint8_t XKAN1_rxfifo0 = 0;
volatile uint8_t XKAN1_rxfifo1 = 0;
...
void XKAN1_IRQHandler(void)
{
    ...
    {
        xKan_RxFifo0Callback(XKAN1, &xkan_parameter_init, &xKan_RxHeader, g_KanRxData, XKAN1_RAM_BASE);
        XKAN1_rxfifo0 += 1;
    }
    ...
    {
        xKan_RxFifo1Callback(XKAN1, &xkan_parameter_init, &xKan_RxHeader, g_KanRxData, XKAN1_RAM_BASE);
        XKAN1_rxfifo1 += 1;
    }
...
{
    uint32_t tmpreg;
    xkan_config(XKAN0, XKAN_LOOPBACK_MODE);
...
{
    uint32_t tmpreg;
    xkan_config(XKAN0, XKAN_SILENT_MODE);
    xkan_config(XKAN1, XKAN_NORMAL_MODE);
...
{
    uint32_t tmpreg;
    xkan_config(XKAN0, XKAN_SILENT_LOOPBACK_MODE);
    xkan_config(XKAN1, XKAN_NORMAL_MODE);
...
void main(void)
{
    ...
    if((XKAN1_rxbuf == 1) && (XKAN1_rxfifo0 == 0) && (XKAN1_rxfifo1 == 0)) {
        simulation_pass();
    } else {
        simulation_fail();
    }
