...
void XKAN1_IRQHandler(void)
{
    if(xKan_GetInterruptStatus(XKAN1, XKAN_CFG_INT_STATUS_RXB_UPD_INT) && xKan_GetInterruptEnable(XKAN1, XKAN_CFG_INT_EN_RXB_UPD_EN))
    {
        ...
        timestamp_r[rx_done] = xKan_GetTimeStamp(XKAN1);
        rx_done ++;
        pRxData += BUFSIZE;
        if (rx_done == 4 ) xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void XKAN0_IRQHandler(void)
{
    ...
    {
        xKan_ClearTxComplete(XKAN0);
        timestamp_t[tx_done] = xKan_GetTimeStamp(XKAN0);
        tx_done ++;
        if (tx_done == 4 ) xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_TX_TRANS_COMPLETE_EN, DISABLE);
    }
}
