...
volatile uint8_t rx1_done = 0;
volatile uint8_t rx2_done = 0;

volatile ErrStatus status = ERROR;
volatile ErrStatus id1_match_status = ERROR;
volatile ErrStatus id2_match_status = ERROR;
...
void XKAN0_config(void)
{
    ...
    xkan1_parameter_init.frame_format = XKAN_FRAME_CLASSIC;
    ...
    xkan1_parameter_init.ram_info.ExtFiltersNbr = 1;
    xkan1_parameter_init.ram_info.RxBuffersNbr = 1;
    xkan1_parameter_init.ram_info.RxBufferSize = XKAN_DATA_BYTES_8;

    xkan1_parameter_init.ram_info.TxBuffersNbr = 2;
    xkan1_parameter_init.ram_info.TxElmtSize = XKAN_DATA_BYTES_8;

    xKan_Init(XKAN0, &xkan1_parameter_init);
    ...
    xKan_ConfigFilter(XKAN0, &xkan1_parameter_init, &xKan_RXFilter, XKAN0_RAM_BASE);
    ...
    xKan_ConfigGlobalFilter(XKAN0, XKAN_REJECT, XKAN_REJECT, XKAN_FILTER_REMOTE, XKAN_REJECT_REMOTE);
}
...
void XKAN1_config(void)
{
    ...
    xkan2_parameter_init.frame_format = XKAN_FRAME_CLASSIC;
    ...
    xkan2_parameter_init.ram_info.ExtFiltersNbr = 1;

    xkan2_parameter_init.ram_info.RxBuffersNbr = 1;
    xkan2_parameter_init.ram_info.RxBufferSize = XKAN_DATA_BYTES_8;
    xkan2_parameter_init.ram_info.TxBuffersNbr = 2;
    xkan2_parameter_init.ram_info.TxElmtSize = XKAN_DATA_BYTES_8;

    xKan_Init(XKAN1, &xkan2_parameter_init);
    ...
    xKan_RXFilter.RxBufferIndex = 0;
    xKan_ConfigFilter(XKAN1, &xkan2_parameter_init, &xKan_RXFilter, XKAN1_RAM_BASE);
    /* step4.2: extend filter config */
    xKan_RXFilter.IdType = XKAN_EXTENDED_ID;
    xKan_RXFilter.FilterIndex = 0;   
    xKan_RXFilter.FilterType = XKAN_FILTER_MASK;
    ...
    */
    xKan_ConfigGlobalFilter(XKAN1, XKAN_REJECT, XKAN_REJECT, XKAN_REJECT_REMOTE, XKAN_FILTER_REMOTE);
}
/**
  * @brief  XKAN0_SendPacket send xkan frame to bus
  ...
  *             XKAN_DLC_BYTES_64 
  */
void XKAN0_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    XKAN_AddMessageToTxBuffer(&xkan1_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER0, XKAN0_RAM_BASE);
    ...
    XKAN_AddMessageToTxBuffer(&xkan1_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1, XKAN0_RAM_BASE);
    xKan_EnableTxBufferRequest(XKAN0, XKAN_TX_BUFFER0|XKAN_TX_BUFFER1);
}

void XKAN1_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    /* step1: cfg send frame */
    xKan_TxHeader.Identifier = 0x014;
    xKan_TxHeader.IdType = XKAN_STANDARD_ID;
    ...
    xKan_TxHeader.BitRateSwitch = XKAN_BRS_OFF; 
    xKan_TxHeader.FDFormat = XKAN_CLASSIC_CAN;
    XKAN_AddMessageToTxBuffer(&xkan2_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER0, XKAN1_RAM_BASE);
    ...
    XKAN_AddMessageToTxBuffer(&xkan2_parameter_init, XKAN1, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1, XKAN1_RAM_BASE);
    xKan_EnableTxBufferRequest(XKAN1, XKAN_TX_BUFFER0|XKAN_TX_BUFFER1);
}
...
void XKAN0_IRQHandler(void)
{
    ...
    {
        xKan_RxBufferNewMessageCallback(XKAN0, &xkan1_parameter_init, &xKan_RxHeader, pRxData, XKAN0_RAM_BASE);
        rx1_done ++;
        if(xKan_RxHeader.Identifier == 0x14) id1_match_status = SUCCESS;
        if (rx1_done == 1 ) xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void XKAN1_IRQHandler(void)
{
    ...
    {
        xKan_RxBufferNewMessageCallback(XKAN1, &xkan2_parameter_init, &xKan_RxHeader, tmpData, XKAN1_RAM_BASE);
        if(xKan_RxHeader.RxFrameType == 0x40000000) XKAN1_SendPacket(tx_msg, XKAN_DLC_BYTES_0);;
        rx2_done ++;
        if(xKan_RxHeader.Identifier == 0x12345678) id2_match_status = SUCCESS;
        if (rx2_done == 1) xKan_EnableInterrupt(XKAN1, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void main(void)
{
    ...
    printf("xkan normal_filter_function_check\n\r");
    /* xkan cfg */ 
    XKAN0_config();
    XKAN1_config();
    ...
    XKAN0_SendPacket(tx_msg, XKAN_DLC_BYTES_0);

    /* wait for read over for rx_buffer */
    while(rx1_done != 1 || rx2_done != 1){}

    /* compare the read buffer and write buffer */
    if((SUCCESS != id2_match_status) || (SUCCESS != id1_match_status)) {
        simulation_fail();
    } else {
        simulation_pass();
    }
