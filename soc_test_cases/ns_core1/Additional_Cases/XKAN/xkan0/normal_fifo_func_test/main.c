...
void xkan_config(void)
{
    ...
    */
    ...
    xKan_ConfigRxFifoOperationMode(XKAN1, XKAN_CFG_RX_FIFO0, XKAN_RXFIFO_OP_BLOCK);
    xKan_ConfigRxFifoOperationMode(XKAN1, XKAN_CFG_RX_FIFO1, XKAN_RXFIFO_OP_OVERWRITE);
}
...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    for(int i = 0; i < FRAMENUM/2; i++) {
        ...
        _DataBuf = tmp_msg0;
        XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, (XKAN_TX_BUFFER0 << 2*i), XKAN0_RAM_BASE);

        xKan_TxHeader.Identifier = 0x12345687;
        xKan_TxHeader.IdType = XKAN_EXTENDED_ID;
        _DataBuf = tmp_msg1;
        XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1 << 2*i, XKAN0_RAM_BASE);
    }
    ...
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, (XKAN_TX_BUFFER0 << 2*5), XKAN0_RAM_BASE);

    xKan_TxHeader.Identifier = 0x12345687;
    xKan_TxHeader.IdType = XKAN_EXTENDED_ID;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1 << 2*5, XKAN0_RAM_BASE);

    /* miss frame */
    _DataBuf = tmp_msg3;
    xKan_TxHeader.Identifier = 0x277;
    xKan_TxHeader.IdType = XKAN_STANDARD_ID;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, (XKAN_TX_BUFFER0 << 2*6), XKAN0_RAM_BASE);

    xKan_TxHeader.Identifier = 0x13456789;
    xKan_TxHeader.IdType = XKAN_EXTENDED_ID;
    XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER1 << 2*6, XKAN0_RAM_BASE);

    xKan_EnableTxBufferRequest(XKAN0, 0x3FFF);
}
...
void main(void)
{
    ...
    /* wait for tx over */
    while(XKAN0->CFG_TXBAR) {}
    while(XKAN0->CFG_TXREQ_PENDING) {}
