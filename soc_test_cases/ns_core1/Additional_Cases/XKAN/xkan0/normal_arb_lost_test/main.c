...
void xkan_config(void)
{
    ...
    /* for bit timing gen */
    XKAN0->CFG_DBTR = ((((uint32_t)0x3) - 1 << 14)  | (((uint32_t)0x4 - 1) << 10) | (((uint32_t)0xb - 1) << 5)  | ((uint32_t)0x2 - 1));
    XKAN1->CFG_DBTR = ((((uint32_t)0x3) - 1 << 14)  | (((uint32_t)0x4 - 1) << 10) | (((uint32_t)0xb - 1) << 5)  | ((uint32_t)0x2 - 1));
    XKAN0->CFG_NBTR = ((((uint32_t)0x3) - 1 << 24)  | (((uint32_t)0x9 - 1) << 17) | (((uint32_t)0x16 - 1) << 9)  | ((uint32_t)0x8 - 1));
...
void xKan_SendPacket(uint8_t *_DataBuf, uint32_t _Len, uint32_t rval)
{
    ...
    case 0:
        ...
        XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, (XKAN_TX_BUFFER0 << rval), XKAN0_RAM_BASE);
    ...
    xKan_EnableTxBufferRequest(XKAN0, (XKAN_TX_BUFFER0 << rval));
