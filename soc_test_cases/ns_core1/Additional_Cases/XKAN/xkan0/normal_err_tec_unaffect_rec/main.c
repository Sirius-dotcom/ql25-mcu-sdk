...
void xKan_SendClassicPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    for(int k = 0; k < 4; k++) {
        ...
        if(k == 3){
            xKan_EnableTxBufferRequest(XKAN0, 0xFFFFFFFE);
            while ((XKAN0->CFG_ESR & XKAN_CFG_ESR_TEC)!= 0xF8);
            goto reset;
        } else {
...
void xKan_SendCanFdPacket(uint8_t *_DataBuf, uint32_t _Len)
{
    ...
    if(_Len == XKAN_DLC_BYTES_1) {
        ...
        while (!(XKAN1->CFG_ESR & XKAN_CFG_ESR_ERR_PASS_FLAG));
    } else {
        XKAN_AddMessageToTxBuffer(&xkan_parameter_init, XKAN0, &xKan_TxHeader, _DataBuf, XKAN_TX_BUFFER17, XKAN1_RAM_BASE);
        xKan_EnableTxBufferRequest(XKAN0, XKAN_TX_BUFFER17);
        while (!(XKAN0->CFG_ESR & XKAN_CFG_ESR_TEC));
        do{
            __NOP();
