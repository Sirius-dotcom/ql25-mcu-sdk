...
uint8_t tx_msg[8];
uint8_t tmp_msg0[8], tmp_msg1[8], tmp_msg2[8], tmp_msg3[8], tmp_msg4[8];
uint8_t tmp_msg5[8], tmp_msg6[8], tmp_msg7[8], tmp_msg8[8], tmp_msg9[8];
...
void xkan_config(void)
{
    ...
    xkan_parameter_init.ram_info.RxFifo0ElmtsNbr = 10;
    ...
    xKan_RXFilter.FilterID1 = 0x13;
    xKan_RXFilter.FilterID2 = 0x14;
    ...
    */
    ...
    xKan_ConfigRxFifoWatermark(XKAN0, XKAN_CFG_RX_FIFO0, FRAMENUM-1);
}
...
void XKAN0_IRQHandler(void)
{
    ...
    {
        for(; rxfifo0_done < FRAMENUM; rxfifo0_done ++){
            xKan_RxFifo0Callback(XKAN0, &xkan_parameter_init, &xKan_RxHeader, pRxData, XKAN0_RAM_BASE);
            pRxData += BUFSIZE;
            if (rxfifo0_done == FRAMENUM) xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_RXF0_WM_EN, DISABLE);
        }
    }

    if(xKan_GetInterruptStatus(XKAN0, XKAN_CFG_INT_STATUS_RXF1_WM_INT) && xKan_GetInterruptEnable(XKAN0, XKAN_CFG_INT_EN_RXF1_WM_EN))
    {
        xKan_RxFifo1Callback(XKAN0, &xkan_parameter_init, &xKan_RxHeader, pRxData, XKAN0_RAM_BASE);
        rxfifo1_done ++;
        pRxData += BUFSIZE;
        if (rxfifo1_done == 4) xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_RXF1_WM_EN, DISABLE);
    }
    ...
    {
        xKan_RxBufferNewMessageCallback(XKAN0, &xkan_parameter_init, &xKan_RxHeader, pRxData, XKAN0_RAM_BASE);
        rx_buf_done ++;
        pRxData += BUFSIZE;
        if (rx_buf_done == 2) xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_RXB_UPD_EN, DISABLE);
    }
}
...
void main(void)
{
    ...
    xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_RXF1_WM_EN, ENABLE);
    xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_RXB_UPD_EN, ENABLE);

    printf("xkan loopback_fifo_sameid_test\n\r");
    /* xkan cfg */ 
    xkan_config();
    ...
    for(int i = 0; i < BUFSIZE; i++){
        ...
        *(pTxData+BUFSIZE*(FRAMENUM-1)) = tmp_msg0[i];
        *(pTxData+BUFSIZE*(FRAMENUM-2)) = tmp_msg1[i];
        *(pTxData+BUFSIZE*(FRAMENUM-3)) = tmp_msg2[i];
        *(pTxData+BUFSIZE*(FRAMENUM-4)) = tmp_msg3[i];
        *(pTxData+BUFSIZE*(FRAMENUM-5)) = tmp_msg4[i];
        *(pTxData+BUFSIZE*(FRAMENUM-6)) = tmp_msg9[i];
        *(pTxData+BUFSIZE*(FRAMENUM-7)) = tmp_msg8[i];
        *(pTxData+BUFSIZE*(FRAMENUM-8)) = tmp_msg7[i];
        *(pTxData+BUFSIZE*(FRAMENUM-9)) = tmp_msg6[i];
        *pTxData ++ = tmp_msg5[i];
    }
