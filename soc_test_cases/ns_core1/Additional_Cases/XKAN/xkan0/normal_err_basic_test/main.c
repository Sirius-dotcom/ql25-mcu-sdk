...
{
    __disable_irq();
    if(xKan_GetInterruptStatus(XKAN0, XKAN_CFG_INT_EN_ERROR_DET_INT_EN) && xKan_GetInterruptEnable(XKAN0, XKAN_CFG_INT_STATUS_ERROR_DET_INT))
    {

        xKan_ClearInterruptStatus(XKAN0, XKAN_CFG_INT_STATUS_ERROR_DET_INT);
        err_det_int_sta = SUCCESS;
        err_cnt ++;
        /* clear pending bit */
        xKan_ClearTxReqPending(XKAN0, (1<<(32-err_cnt)));
        if(err_cnt == 16) {
            err_tec = (XKAN0->CFG_ESR & XKAN_CFG_ESR_TEC);
            err_passive = (XKAN0->CFG_ESR & XKAN_CFG_ESR_ERR_PASS_FLAG);
            if (err_tec == 128 && err_passive){
                passive_sta = SUCCESS;
            }
        }
        if(err_cnt == 32) {
            err_code = (XKAN0->CFG_ESR & XKAN_CFG_ESR_LEC_MASK) >> XKAN_CFG_ESR_LEC_OFS;
            xKan_ClearErrCode(XKAN0);
            xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_ERROR_DET_INT_EN, DISABLE);
        }
    }

    if(xKan_GetInterruptStatus(XKAN0, XKAN_CFG_INT_EN_BUS_OFF_INT_EN) && xKan_GetInterruptEnable(XKAN0, XKAN_CFG_INT_STATUS_BUS_OFF_INT))
    {
        xKan_ClearInterruptStatus(XKAN0, XKAN_CFG_INT_STATUS_BUS_OFF_INT);
        xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_BUS_OFF_INT_EN, DISABLE);
        err_tec = (XKAN0->CFG_ESR & XKAN_CFG_ESR_TEC);
        if (err_tec == 0) {
...
void main(void)
{
    ...
    ECLIC_Register_IRQ(XKAN0_IRQn, ECLIC_NON_VECTOR_INTERRUPT,
                                    ...
                                    XKAN0_IRQHandler);
    xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_ERROR_DET_INT_EN, ENABLE);
    xKan_EnableInterrupt(XKAN0, XKAN_CFG_INT_EN_BUS_OFF_INT_EN, ENABLE);
