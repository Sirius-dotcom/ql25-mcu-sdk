...
{
    if ((LGPIO_GetITStatus(LGPIO1, pin_source[0], LGPIO_IT_RISE) == pin_source[0]) && ((LGPIO1->PAD_GRP0_RISE_IE & pin_source[0]) == pin_source[0])) {
        LGPIO_ITClear(LGPIO1, pin_source[0], LGPIO_IT_RISE);
        IRQ_status++;
    }
}
...
int main(void)
{
    ...
    LGPIO_WriteBit(LGPIO1,pin_source[1],0);
    LGPIO_Mode(LGPIO1,pin_source[1],PP);
    LGPIO_PULLMode(LGPIO1,pin_source[1],PDE);
    ...
    LGPIO_ITConfig(LGPIO1, pin_source[0], LGPIO_IT_RISE, ENABLE);

    delay_1ms(1);

    LGPIO_WriteBit(LGPIO1,pin_source[1],1);
    LGPIO_Mode(LGPIO1,pin_source[1],PP);
    LGPIO_PULLMode(LGPIO1,pin_source[1],PDE);
